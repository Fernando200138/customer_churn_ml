...
def expressionToAST(ex):
    ...
    """
    return ASTNode(
        ex.astType, ex.astKind, ex.value, [expressionToAST(c) for c in ex.children]
    )
...
def typeCompileAst(ast):
        ...
        for sig in sigPerms(basesig):
            ...
            for sig in sigPerms(basesig):
                ...
                if funcname in interpreter.funccodes:
                    ...
                    children += [
                        ASTNode("raw", "none", interpreter.funccodes[funcname])
                    ]
                    break
            else:
    ...
    return ASTNode(
        ast.astType, ast.astKind, value, [typeCompileAst(c) for c in children]
    )
...
def getConstants(ast):
    ...
    constants = [convertConstantToKind(a.value, a.astKind) for a in constants_order]
...
def optimizeTemporariesAllocation(ast):
    ...
    node_regs = dict(
        (n, set(c.reg for c in n.children if c.reg.temporary)) for n in nodes
    )
...
def convertASTtoThreeAddrForm(ast):
    ...
    """
    return [
        (node.value, node.reg) + tuple([c.reg for c in node.children])
...
def precompile(ex, signature=(), context={}, sanitize: bool = True):
    ...
    signature = "".join(
        type_to_typecode[types.get(x, default_type)] for x in input_names
    )
...
def NumExpr(ex, signature=(), sanitize: bool = True, **kwargs):
    ...
    threeAddrProgram, inputsig, tempsig, constants, input_names = precompile(
        ex, signature, context, sanitize=sanitize
    )
    program = compileThreeAddrForm(threeAddrProgram)
    return interpreter.NumExpr(
        inputsig.encode("ascii"),
    ...
    def getArg(pc, offset):
        ...
        if code != b"n":
            if arg == 0:
                ...
                return ("c%d[%s]" % (arg, nex.constants[arg - r_constants])).encode(
                    "ascii"
                )
            else:
    ...
    **kwargs,
    ...
    try:
        ...
        arguments = getArguments(
            names, local_dict, global_dict, _frame_depth=_frame_depth
        )
        ...
        try:
            ...
            compiled_ex = _numexpr_cache.c[numexpr_key] = NumExpr(
                ex, signature, sanitize=sanitize, **context
            )
        kwargs = {
            "out": out,
            "order": order,
...
def evaluate(
    ...
    **kwargs,
    ...
    if e is None:
        return re_evaluate(
            local_dict=local_dict, global_dict=global_dict, _frame_depth=_frame_depth
        )
    else:
        raise e
...
def re_evaluate(
    ...
    try:
        ...
        raise RuntimeError(
            "A previous evaluate() execution was not found, please call `validate` or `evaluate` once before `re_evaluate`"
        )
