...
def _init_num_threads():
    ...
    if (
        "NUMEXPR_MAX_THREADS" in os.environ
        and os.environ["NUMEXPR_MAX_THREADS"] != ""
        ...
        if n_cores > 16:
            ...
            log.info(
                'Note: NumExpr detected %d cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.'
                % n_cores
            )
            n_cores = 16
...
class ContextDict:
    ...
    def update(self, *args, **kwargs):
        ...
        if args:
            if len(args) > 1:
                raise TypeError(
                    f"update() takes at most 1 positional argument ({len(args)} given)"
                )
            other = args[0]
