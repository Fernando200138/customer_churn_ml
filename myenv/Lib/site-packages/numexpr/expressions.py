...
def div_op(a, b):
    if get_optimization() in ("moderate", "aggressive"):
        if (
            isinstance(b, ConstantNode)
            and (a.astKind == b.astKind)
            and a.astKind in ("float", "double", "complex")
        ):
...
def pow_op(a, b):
    ...
    if isinstance(b, ConstantNode):
        x = b.value
        if a.astKind in ("int", "long") and b.astKind in ("int", "long") and x < 0:
...
class ExpressionNode:
    ...
    def __bool__(self):
        raise TypeError(
            "You can't use Python's standard boolean operators in "
...
class FuncNode(OpNode):
    def __init__(self, opcode=None, args=None, kind=None):
        ...
        if opcode in (
            ...
            "isinf",
            "signbit",
        ):  # bodge for boolean return functions
            kind = "bool"
