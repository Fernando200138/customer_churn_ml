...
import numexpr
from numexpr import E, NumExpr, disassemble, evaluate, re_evaluate, use_vml, validate
...
class test_numexpr(TestCase):
    ...
    def test_reductions(self):
        ...
        x = zeros(100000) + 0.01  # checks issue #41
    ...
    def test_in_place(self):
        x = arange(10000.0).reshape(1000, 10)
        evaluate("x + 3", out=x)
        assert_equal(x, arange(10000.0).reshape(1000, 10) + 3)
        y = arange(10)
        evaluate("(x - 3) * y + (x - 3)", out=x)
        assert_equal(x, arange(10000.0).reshape(1000, 10) * (arange(10) + 1))
    ...
    def test_str_contains_basic3(self):
        haystacks = array([b"abckkk", b"adef", b"xyz", b"x11abcp", b"za", b"abc"])
        needles = array([b"abc", b"def", b"aterr", b"oot", b"zu", b"ab"])
    ...
    def test_str_contains_basic4(self):
        needles = array(
            [b"abc", b"def", b"aterr", b"oot", b"zu", b"ab c", b" abc", b"abc "]
        )
    ...
    def test_str_contains_basic5(self):
        needles = array([b"abc", b"ab c", b" abc", b" abc ", b"\tabc", b"c h"])
        res = evaluate('contains(b"test abc here", needles)')
    ...
    @unittest.skipIf(
        hasattr(sys, "pypy_version_info"), "PyPy does not have sys.getrefcount()"
    )
...
class test_evaluate(TestCase):
    ...
    def test_re_evaluate_dict(self):
        ...
        b1 = array([4.0, 5.0, 6.0])
        c1 = array([7.0, 8.0, 9.0])
    ...
    def test_complex_tan(self):
        ...
        x = np.arange(1, 400.0, step=16.0, dtype=np.complex128)
        y = 1j * np.arange(1, 400.0, step=16.0, dtype=np.complex128)
    ...
    def test_recarray_strides(self):
        ...
        recarr["f1"] = b
        c = recarr["f1"]
        assert_array_almost_equal(evaluate("sqrt(c) > 1."), sqrt(c) > 1.0)
    ...
    def test_disassemble(self):
        assert_equal(
            disassemble(NumExpr("where(m, a, -1)", [("m", bool), ("a", float)])),
            [
    ...
    def test_nan_constant(self):
        ...
        assert_equal(
            NumExpr(expr, [("a", double), ("b", double)]).constants, (float("nan"),)
        )
    ...
    def test_f32_constant(self):
        ...
        assert_array_equal(
            NumExpr(ConstantNode(numpy.float32(1))).run(),
...
for n in (-7, -2.5, -1.5, -1.3, -0.5, 0, 0.0, 1, 2.3, 2.5, 3):
...
def equal(a, b, exact):
    ...
    if exact:
        ...
        if hasattr(a, "dtype") and a.dtype == "f4":
            ...
            atol = 1e-8
        return shape(a) == shape(b) and allclose(ravel(a), ravel(b), atol=atol)
...
def test_expressions(expr, test_scalar, dtype, optimization, exact, section_name):
    array_size = 100
    ...
    def test_omp_num_threads_empty_string(self):
        with _environment("OMP_NUM_THREADS", ""):
            if "sparc" in platform.machine():
                ...
                self.assertEqual(
                    min(detect_number_of_cores(), MAX_THREADS),
...
class test_threading(TestCase):
    ...
    def test_multithread(self):
        ...
        nthreads = 30
        threads = [threading.Thread(target=work, args=(1e5,)) for i in range(nthreads)]
        for t in threads:
            t.start()
        for t in threads:
            t.join()
    ...
    def test_thread_safety(self):
        ...
        def thread_function(
            a_value, b_value, expression, expected_result, results, index
        ):
                ...
                (
                    ...
                    if expr == "a + b"
                        ...
                        if expr == "a - b"
                            ...
                            a_value[i] * b_value[i]
                            if expr == "a * b"
                            else a_value[i] / b_value[i] if expr == "a / b" else None
                        )
                    )
                )
...
def print_versions():
    ...
    print(
        "Number of threads used by default: %d "
...
def suite():
    ...
    for n in range(niter):
        ...
        if "sparc" not in platform.machine():
            theSuite.addTest(
                unittest.defaultTestLoader.loadTestsFromTestCase(test_numexpr2)
            )
        theSuite.addTest(
            unittest.defaultTestLoader.loadTestsFromTestCase(test_evaluate)
        )
        ...
        theSuite.addTest(
            unittest.defaultTestLoader.loadTestsFromTestCase(test_int32_int64)
        )
        theSuite.addTest(
            unittest.defaultTestLoader.loadTestsFromTestCase(test_uint32_int64)
        )
        ...
        theSuite.addTest(
            unittest.defaultTestLoader.loadTestsFromTestCase(test_threading_config)
        )

        # multiprocessing module is not supported on Hurd/kFreeBSD
        if pl.system().lower() not in ("gnu", "gnu/kfreebsd"):
            theSuite.addTest(
                unittest.defaultTestLoader.loadTestsFromTestCase(test_subprocess)
            )
        ...
        theSuite.addTest(
            unittest.defaultTestLoader.loadTestsFromTestCase(test_threading)
        )
