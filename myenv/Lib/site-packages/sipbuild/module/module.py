...
from .abi_version import (
    get_latest_version,
    get_module_source_dir,
    get_source_version_range,
    parse_abi_version,
)


def module(
    sip_module, abi_version, project, sdist, setup_cfg, sip_h, sip_rst, target_dir
):
...
def copy_sip_h(abi_version, target_dir, sip_module="", version_info=True):
    ...
    patches = _create_patches(sip_module, module_source_dir, version_info=version_info)
...
def _create_patches(sip_module, module_source_dir, project="", version_info=True):
    ...
    for name in os.listdir(module_source_dir):
        ...
        if name != "MANIFEST.in" and name.endswith(".in"):
            ...
            if name != "README" or setup_cfg is None:
                _install_source_file(name, module_source_dir, sdist_dir, patches)
        else:
...
def _install_source_file(name, module_source_dir, target_dir, patches):
    ...
    _install_file(
        os.path.join(module_source_dir, name) + ".in",
