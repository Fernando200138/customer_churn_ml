...
class Project(AbstractProject, Configurable):
    ...
    _options = (
        ...
        Option("quiet", option_type=bool, help="disable all progress messages"),
        Option("verbose", option_type=bool, help="enable verbose progress messages"),
        Option(
            "name",
            help="the name used in sdist and wheel file names",
            metavar="NAME",
            tools=["sdist", "wheel"],
        ),
        Option("abi_version", help="The ABI to generate code for", metavar="M[.N]"),
        Option("build_dir", help="the build directory", metavar="DIR"),
        Option(
            "build_tag",
            help="the build tag to be used in the wheel name",
            metavar="TAG",
            tools=["wheel"],
        ),
        Option(
            ...
            option_type=bool,
            inverted=True,
            help="don't create a .dist-info directory",
            tools=["install"],
        ),
        Option(
            "manylinux",
        ...
        Option(
            ...
            help="the scripts installation directory",
            metavar="DIR",
            tools=["build", "install"],
        ),
        Option(
            "target_dir",
            default=sysconfig.get_path("platlib"),
            help="the target installation directory",
            metavar="DIR",
            tools=["build", "install"],
        ),
        Option("api_dir", help="generate a QScintilla .api file in DIR", metavar="DIR"),
        Option(
            "compile",
            option_type=bool,
            inverted=True,
        ...
        Option(
            ...
            option_type=bool,
            inverted=True,
            help="disable any reference to the SIP version number in " "generated code",
            tools=["build"],
        ),
    )
    ...
    _multibindings_options = (
        Option(
            "disable",
            option_type=list,
            help="disable the NAME bindings",
            metavar="NAME",
        ),
        Option(
            "enable", option_type=list, help="enable the NAME bindings", metavar="NAME"
        ),
    )
    ...
    def apply_nonuser_defaults(self, tool):
        ...
        if self.bindings_factory is None:
            ...
            self.bindings_factory = self.import_callable(
                self.bindings_factory, Bindings
            )
        ...
        if self.builder_factory is None:
            ...
            # Convert the name to a callable.
            self.builder_factory = self.import_callable(
                self.builder_factory, AbstractBuilder
            )
    ...
    def get_platform_tag(self):
        ...
        if self.py_platform == "darwin":
            ...
            if len(parts) == 3:
                ...
                if self.all_modules_use_limited_abi and (min_major, min_minor) < (
                    MACOS_MIN_OS_MAJOR,
                    MACOS_MIN_OS_MINOR,
                ):
    ...
    def get_sip_distinfo_command_line(
        self,
        sip_distinfo,
        inventory,
        generator=None,
        ...
        if value:
            try:
                ...
                raise PyProjectOptionException(
                    ...
                    "'{0}' is an invalid GLIBC version number".format(value),
        ...
        if value:
            try:
                ...
                raise PyProjectOptionException(
                    ...
                    "'{0}' is an invalid macOS version number".format(value),
                    section_name="tool.sip.project",
                )
    ...
    def setup(self, pyproject, tool, tool_description):
        ...
        if pyproject.pyproject is None:
            raise PyProjectException("there is no such file in the current directory")
    ...
    def verify_configuration(self, tool):
        ...
        self.sip_include_dirs = [self.project_path(d) for d in self.sip_include_dirs]
        ...
        if tool == "wheel" and self.api_dir:
            if os.path.isabs(self.api_dir) or os.path.dirname(self.api_dir) == "..":
                raise PyProjectOptionException(
                    "api-dir", "must be relative when building a wheel"
                )
    ...
    def _configure_from_arguments(self, tool, tool_description):
        ...
        parser = ArgumentParser(
            tool_description, build_tool=True, argument_default=SUPPRESS
        )
        ...
        self.add_command_line_options(parser, tool, all_options, options=options)
        ...
        for option, configurables in all_options.items():
            for configurable in configurables:
                if hasattr(args, option.dest):
                    setattr(configurable, option.name, getattr(args, option.dest))
    ...
    def _set_initial_configuration(self, pyproject, tool):
        ...
        try:
            ...
            raise PyProjectOptionException(
                "version",
                "'{0}' is an invalid version number".format(self.version_str),
        ...
        for bindings in self.bindings.values():
            bindings.configure(pyproject, "tool.sip.bindings." + bindings.name, tool)
    ...
    def _get_limited_abi_version_str(self):
        ...
        try:
            ...
            min_req_version = next(spec_set.filter((f"3.{v}" for v in range(100))))

            min_req_version = packaging.version.parse(min_req_version)
            limited_abi_version_str = "0x{0:02x}{1:02x}{2:02x}00".format(
                min_req_version.major, min_req_version.minor, min_req_version.micro
            )
        except Exception as e:
            # Default to the oldest version of Python we support.
            limited_abi_version_str = "0x03{0:02x}0000".format(OLDEST_SUPPORTED_MINOR)
