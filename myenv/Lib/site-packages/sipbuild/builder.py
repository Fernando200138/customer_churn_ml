...
class Builder(AbstractBuilder):
    ...
    def build_sdist(self, sdist_directory):
        ...
        for dname, dirnames, filenames in os.walk(project.root_dir):
            ...
            for s_fn in filenames:
                ...
                d_fn_path = os.path.join(
                    sdist_root, os.path.relpath(s_fn_path, project.root_dir)
                )
        ...
        write_metadata(
            project.metadata,
            project.get_requires_dists(),
            os.path.join(sdist_root, "PKG-INFO"),
            project.root_dir,
        )
    ...
    def build_wheel(self, wheel_directory):
        ...
        if project.all_modules_use_limited_abi:
            ...
            major_minor = "{}{}".format(
                (sys.hexversion >> 24) & 0xFF, (sys.hexversion >> 16) & 0xFF
            )
    ...
    def _generate_bindings(self):
        ...
        if project.sip_module:
            ...
            sip_include_dirs.append(
                os.path.join(project.target_dir, project.get_bindings_dir())
            )
        ...
        for bindings in project.bindings.values():
            project.progress("Generating the {0} bindings".format(bindings.name))
        ...
        if project.dunder_init:
            ...
            for bindings in project.bindings.values():
                if bindings.pep484_pyi:
                    ...
                    installable = Installable("sip_pyi", target_subdir=package_dir)
                    installable.files.append(os.path.join(project.build_dir, "sip.pyi"))
                    project.installables.append(installable)
                    ...
                    installable = Installable("py_typed", target_subdir=package_dir)
