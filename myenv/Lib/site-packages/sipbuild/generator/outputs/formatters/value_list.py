...
def fmt_value_list_as_py_expression(spec, value_list, embedded=False, as_xml=False):
    """The Python representation of a value list as an expression."""

    return _expression(
        spec, value_list, as_python=True, embedded=embedded, as_xml=as_xml
    )
        ...
        for member in enum.members:
            if member.cpp_name == target_base_name:
                if enum.is_scoped:
                    ...
                    if (enum.scope is None and target_scope is None) or (
                        enum.scope is not None
                        and target_scope is not None
                        and enum.scope.iface_file.fq_cpp_name == target_scope
                    ):
                        return fmt_enum_member_as_rest_ref(member)
...
def _expression(spec, value_list, as_python=False, embedded=False, as_xml=False):
    ...
    for value in value_list:
        ...
        if value.value_type is ValueType.QCHAR:
            ...
            args = [
                _expression(
                    spec, arg, as_python=as_python, embedded=embedded, as_xml=as_xml
                )
