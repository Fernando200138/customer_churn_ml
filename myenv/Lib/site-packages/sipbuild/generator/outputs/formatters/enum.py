...
def fmt_enum_as_type_hint(spec, enum, defined):
    ...
    if enum.scope is None:
        ...
        outer_scope = (
            enum.scope.scope if scope_iface.type is IfaceFileType.CLASS else None
        )

        is_defined = iface_is_defined(scope_iface, outer_scope, module, defined)
    ...
    module_name = (
        enum.module.py_name + "."
        if enum.module is not module and defined is not None
        else ""
    )
