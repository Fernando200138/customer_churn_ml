...
def fmt_argument_as_name(spec, arg, arg_nr):
    ...
    if (
        spec.module.use_arg_names
        and arg.name is not None
        and arg.type is not ArgumentType.ELLIPSIS
    ):
...
def fmt_argument_as_cpp_type(
    ...
    if (
        use_typename
        and original_typedef is not None
        and not original_typedef.no_type_name
        and arg.array is not ArrayArgument.ARRAY_SIZE
    ):
        ...
        if arg.type is ArgumentType.FUNCTION:
            ...
            s += fmt_argument_as_cpp_type(
                spec, arg.definition.result, scope=scope, strip=strip, as_xml=as_xml
            )

            s += " (" + "*" * nr_derefs + name + ")("

            s += fmt_signature_as_cpp_declaration(
                spec, arg.definition, scope=scope, as_xml=as_xml
            )
        ...
        if arg.type in (ArgumentType.SBYTE, ArgumentType.SSTRING):
            ...
            s += fmt_argument_as_cpp_type(
                spec, arg.definition.type, scope=scope, strip=strip, as_xml=as_xml
            )
            ...
            if spec.c_bindings:
                s += (
                    "union "
                    if arg.definition.class_key is ClassKey.UNION
                    else "struct "
                )

            s += fmt_class_as_scoped_name(
                spec,
                arg.definition,
                scope=scope,
                strip=strip,
            ...
            s += fmt_template_as_cpp_type(
                spec, arg.definition, strip=strip, as_xml=as_xml
            )

        elif arg.type is ArgumentType.ENUM:
            from .enum import fmt_enum_as_cpp_type

            s += fmt_enum_as_cpp_type(
                arg.definition, make_public=make_public, strip=strip
            )
...
def fmt_argument_as_py_default_value(
    spec, arg, type_name, embedded=False, as_xml=False
):
    ...
    if (
        len(arg.default_value) == 1
        and arg.default_value[0].value_type is ValueType.NUMERIC
    ):
        value = arg.default_value[0].value

        if value == 0 and (
            "voidptr" in type_name
            or len(arg.derefs) != 0
            or arg.type in _IMPLICIT_POINTERS
        ):
            return "None"
    ...
    return fmt_value_list_as_py_expression(
        spec, arg.default_value, embedded=embedded, as_xml=as_xml
    )


def fmt_argument_as_py_type(spec, arg, pep484=False, default_value=False, as_xml=False):
...
def fmt_argument_as_rest_ref(spec, arg, out, as_xml=False):
    ...
    if hint is None:
        ...
        s += TypeHintManager(spec).as_rest_ref(hint, out, context, as_xml=as_xml)
...
def fmt_argument_as_type_hint(spec, arg, defined, arg_nr=-1):
    ...
    allow_none = arg.allow_none or (
        arg.type in (ArgumentType.CLASS, ArgumentType.MAPPED)
        and arg.definition.handles_none
    )
    ...
    if hint is None:
        if arg.type is ArgumentType.CLASS:
            ...
            if arg.definition.py_name is not None:
                ...
                type_name = fmt_enum_as_type_hint(spec, arg.definition, defined)
            else:
                type_name = "int"
        elif arg.type is ArgumentType.MAPPED:
            # There would normally be a type hint.
        ...
        if pep484:
            type_name = type_hint_manager.as_type_hint(hint, out, context, defined)
        else:
    ...
    if arg_nr is not None and arg_nr >= 0 and arg.default_value is not None:
        ...
        s += (
            "..."
            if pep484
            else fmt_argument_as_py_default_value(spec, arg, type_name, embedded=True)
        )
