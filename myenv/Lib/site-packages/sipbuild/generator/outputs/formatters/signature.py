...
def fmt_signature_as_cpp_declaration(
    spec, cpp_signature, scope=None, strip=STRIP_NONE, make_public=False, as_xml=False
):
    ...
    args = [
        fmt_argument_as_cpp_type(
            spec, arg, scope=scope, strip=strip, make_public=make_public, as_xml=as_xml
        )
...
def fmt_signature_as_cpp_definition(spec, cpp_signature, scope=None, make_public=False):
    """Return the C++ representation of a signature's arguments as a
    definition.
    """
    ...
    for arg_nr, arg in enumerate(cpp_signature.args):
        ...
        args.append(
            fmt_argument_as_cpp_type(
                spec, arg, name=arg_name, scope=scope, make_public=make_public
            )
        )
...
def fmt_signature_as_type_hint(
    spec, py_signature, need_self=True, exclude_result=False, defined=None
):
    ...
    for arg_nr, arg in enumerate(py_signature.args):
        ...
        if arg.is_in:
            type_hint = fmt_argument_as_type_hint(spec, arg, defined, arg_nr=arg_nr)
            if type_hint:
    ...
    if result is None or (result.type is ArgumentType.VOID and len(result.derefs) == 0):
        ...
        if (
            ...
            and type_hints.hint_out is not None
            and type_hints.hint_out == ""
        ):
            is_result = False
        else:
            is_result = True
