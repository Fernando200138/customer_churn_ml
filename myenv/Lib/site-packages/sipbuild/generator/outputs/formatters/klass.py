...
def fmt_class_as_scoped_name(
    spec, klass, scope=None, strip=STRIP_NONE, make_public=False, as_xml=False
):
    """Return a class's scoped name."""

    if klass.no_type_name:
        return fmt_template_as_cpp_type(
            spec, klass.template, strip=strip, as_xml=as_xml
        )
...
def fmt_class_as_type_hint(spec, klass, defined):
    ...
    quote = (
        ""
        if klass.external
        or iface_is_defined(klass.iface_file, klass.scope, module, defined)
        else "'"
    )

    # Include the module name if it is not the current one.
    module_name = (
        klass.iface_file.module.py_name + "."
        if klass.iface_file.module is not module and defined is not None
        else ""
    )
