...
from ...python_slots import (
    is_hash_return_slot,
    is_inplace_number_slot,
    is_inplace_sequence_slot,
    is_int_arg_slot,
    is_int_return_slot,
    is_multi_arg_slot,
    is_number_slot,
    is_rich_compare_slot,
    is_ssize_return_slot,
    is_void_return_slot,
    is_zero_arg_slot,
)
...
from ..formatters import (
    ...
    fmt_argument_as_name,
    fmt_class_as_scoped_name,
    fmt_copying,
    fmt_enum_as_cpp_type,
    fmt_scoped_py_name,
    fmt_signature_as_cpp_declaration,
    fmt_signature_as_cpp_definition,
    fmt_signature_as_type_hint,
...
def output_code(spec, bindings, project, buildable):
    ...
    if spec.is_composite:
        source_name = os.path.join(
            buildable.build_dir, "sip" + module.py_name + "cmodule.c"
        )

        with CompilationUnit(
            source_name,
...
"""
    ...
    for imported_module in module.all_imports:
        ...
        if len(imported_module.needed_types) != 0:
            sf.write(
                f"extern sipImportedTypeDef sipImportedTypes_{module_name}_{imported_module_name}[];\n"
            )

        if imported_module.nr_virtual_error_handlers != 0:
            sf.write(
                f"extern sipImportedVirtErrorHandlerDef sipImportedVirtErrorHandlers_{module_name}_{imported_module_name}[];\n"
            )

        if imported_module.nr_exceptions != 0:
            sf.write(
                f"extern sipImportedExceptionDef sipImportedExceptions_{module_name}_{imported_module_name}[];\n"
            )
...
def _make_part_name(buildable, module_name, part_nr, source_suffix):
    ...
    return os.path.join(
        buildable.build_dir, f"sip{module_name}part{part_nr}{source_suffix}"
    )
...
def _module_code(spec, bindings, project, py_debug, buildable):
    ...
    if parts:
        ...
        for iface_file in spec.iface_files:
            if (
                iface_file.module is module
                and iface_file.type is not IfaceFileType.EXCEPTION
            ):
                nr_files += 1
        ...
        source_name = os.path.join(
            buildable.build_dir, "sip" + module_name + "cmodule" + source_suffix
        )

    sf = CompilationUnit(source_name, "Module code.", module, project, buildable)
...
"""
    ...
    for virtual_error_handler in spec.virtual_error_handlers:
        if virtual_error_handler.module is module:
            ...
            state_name = _use_in_code(virtual_error_handler.code, "sipGILState")
...
"""
        ...
        for klass in module.proxies:
            if len(klass.ctors) != 0:
                ...
                sf.write(
                    f"    {{{first_field}init_type_{klass_name}, {encoded_type}, SIP_NULLPTR}},\n"
                )
...
static sipPySlotExtenderDef slotExtenders[] = {\n"""
        ...
        for klass in module.proxies:
            for member in klass.members:
                ...
                sf.write(
                    f"    {{(void *)slot_{klass_name}_{member_name}, {slot_name}, {encoded_type}}},\n"
                )
...
"""
        ...
        for member in enum.slots:
            if member.py_slot is not None:
                ...
                sf.write(
                    f"    {{(void *)slot_{enum_name}_{member_name}, {slot_name}}},\n"
                )
...
"""
        ...
        for typedef in spec.typedefs:
            ...
            if typedef.type.type is ArgumentType.LONGLONG:
                ...
                sf.write(
                    fmt_argument_as_cpp_type(
                        spec, typedef.type, strip=STRIP_GLOBAL, use_typename=False
                    )
                )
...
"""
            ...
            for i in range(imported_module.nr_exceptions):
                for exception in spec.exceptions:
                    if (
                        exception.iface_file.module is imported_module
                        and exception.exception_nr == i
                    ):
...
"""
        ...
        for imported_module in module.all_imports:
            ...
            if imported_module.nr_virtual_error_handlers != 0:
                handlers = (
                    f"sipImportedVirtErrorHandlers_{module_name}_{imported_module_name}"
                )

            if imported_module.nr_exceptions != 0:
                exceptions = (
                    f"sipImportedExceptions_{module_name}_{imported_module_name}"
                )

            sf.write(
                f'    {{"{imported_module.fq_py_name}", {types}, {handlers}, {exceptions}}},\n'
            )
...
"""
        ...
        for klass in spec.classes:
            ...
            sf.write(
                f"    {{sipSubClass_{klass_name}, {encoded_type}, SIP_NULLPTR}},\n"
            )
...
"""
    ...
    if module.license is not None:
        ...
        licensee = (
            "SIP_NULLPTR" if license.licensee is None else '"' + license.licensee + '"'
        )
        timestamp = (
            "SIP_NULLPTR"
            if license.timestamp is None
            else '"' + license.timestamp + '"'
        )
        signature = (
            "SIP_NULLPTR"
            if license.signature is None
            else '"' + license.signature + '"'
        )
...
"""
    ...
    exported_types = _optional_ptr(
        len(module.needed_types) != 0, "sipExportedTypes_" + module_name
    )
...
"""
    ...
    veh_table = _optional_ptr(has_virtual_error_handlers, "virtErrorHandlersTable")
    ...
    unsigned_long_instances = _optional_ptr(is_inst_ulong, "unsignedLongInstances")
    long_long_instances = _optional_ptr(is_inst_longlong, "longLongInstances")
    unsigned_long_long_instances = _optional_ptr(
        is_inst_ulonglong, "unsignedLongLongInstances"
    )
    double_instances = _optional_ptr(is_inst_double, "doubleInstances")
    module_license = _optional_ptr(module.license is not None, "&module_license")
    exported_exceptions = _optional_ptr(
        module.nr_exceptions > 0, "sipExportedExceptions_" + module_name
    )
...
"""
    ...
    exception_handler = _optional_ptr(
        (
            _abi_has_next_exception_handler(spec)
            and bindings.exceptions
            and module.nr_exceptions > 0
        ),
...
"""
    ...
    for exception in spec.exceptions:
        ...
        if exception.builtin_base_exception is not None:
            ...
            exception_type = (
                "sipException_"
                + exception.defined_base_exception.iface_file.fq_cpp_name.as_word
            )
...
"""
    ...
    if _pyqt6(spec):
        ...
        for mapped_type in spec.mapped_types:
            ...
            mapped_type_type = fmt_argument_as_cpp_type(
                spec, mapped_type.type, plain=True, no_derefs=True
            )
        ...
        if (
            ...
            if parts:
                if files_in_part == max_per_part:
                    ...
                    source_name = _make_part_name(
                        buildable, module_name, this_part, source_suffix
                    )
                    sf = CompilationUnit(
                        source_name, "Module code.", module, project, buildable
                    )
    ...
    for needed_type in module.needed_types:
        if needed_type.type is ArgumentType.CLASS:
            ...
            if klass.external:
                ...
                sf.write(
                    f"    &sipTypeDef_{module_name}_{klass.iface_file.fq_cpp_name.as_word}.ctd_base,\n"
                )
            ...
            sf.write(
                f"    &sipTypeDef_{module_name}_{mapped_type.iface_file.fq_cpp_name.as_word}.mtd_base,\n"
            )
...
def _ordinary_function(sf, spec, bindings, member, scope=None):
    ...
    if scope is None:
        ...
        py_scope_prefix = (
            "" if py_scope is None else py_scope.iface_file.fq_cpp_name.as_word + "_"
        )
    ...
    if _has_member_docstring(bindings, member, overloads):
        ...
        has_auto_docstring = _member_docstring(sf, spec, bindings, member, overloads)
        sf.write('");\n\n')
    else:
    ...
    if py_scope is None:
        if not spec.c_bindings:
            sf.write(
                f'extern "C" {{static PyObject *func_{member_name}(PyObject *, PyObject *{kw_fw_decl});}}\n'
            )
            sip_self = ""
        else:
            sip_self = "sipSelf"
        ...
        sf.write(
            f"static PyObject *func_{member_name}(PyObject *{sip_self}, PyObject *sipArgs{kw_decl})\n"
        )
    else:
        if not spec.c_bindings:
            sf.write(
                f'extern "C" {{static PyObject *meth_{py_scope_prefix}{member_name}(PyObject *, PyObject *{kw_fw_decl});}}\n'
            )

        sf.write(
            f"static PyObject *meth_{py_scope_prefix}{member_name}(PyObject *, PyObject *sipArgs{kw_decl})\n"
        )
...
def _enum_member_table(sf, spec, scope=None):
    ...
    for enum in spec.enums:
        ...
        if isinstance(scope, WrappedClass):
            ...
            if enum_py_scope is not scope or (
                enum.is_protected and not scope.has_shadow
            ):
                continue
            ...
            enum_py_scope is not None
            or isinstance(enum.scope, MappedType)
            or enum.fq_cpp_name is None
        ):
            continue
...
def _types_inline(sf, spec):
    ...
    for variable in spec.variables:
        ...
        if (
            spec.c_bindings
            or variable.access_code is not None
            or len(variable.type.derefs) != 0
        ):
            continue
...
"""
        ...
        if _py_scope(variable.scope) is None:
            ...
            dict_name = (
                f"(PyObject *)sipTypeAsPyTypeObject({_gto_name(variable.scope)})"
            )
        ...
        if variable.type.is_const:
            type_name = fmt_argument_as_cpp_type(
                spec, variable.type, plain=True, no_derefs=True
            )
            ptr = f"const_cast<{type_name} *>({ptr})"

        sf.write(
            f"    sipAddTypeInstance({dict_name}, {py_name}, {ptr}, {_gto_name(variable.type.definition)});\n"
        )
...
def _class_instances(sf, spec, scope=None):
    ...
    for variable in _variables_in_scope(spec, scope):
        if variable.type.type is not ArgumentType.CLASS and (
            variable.type.type is not ArgumentType.ENUM
            or variable.type.definition.fq_cpp_name is None
        ):
            continue
        ...
        if (
            not spec.c_bindings
            and variable.access_code is None
            and len(variable.type.derefs) == 0
        ):
            continue
...
def _void_pointer_instances(sf, spec, scope=None):
    ...
    for variable in _variables_in_scope(spec, scope):
        ...
        vi_val = _const_cast(
            spec, variable.type, variable.fq_cpp_name.cpp_stripped(STRIP_GLOBAL)
        )
...
def _char_instances(sf, spec, scope=None):
    ...
    for variable in _variables_in_scope(spec, scope):
        if (
            variable.type.type
            not in (
                ArgumentType.ASCII_STRING,
                ArgumentType.LATIN1_STRING,
...
def _string_instances(sf, spec, scope=None):
    ...
    for variable in _variables_in_scope(spec, scope):
        if (
            ...
            or len(variable.type.derefs) == 0
        ) and variable.type.type is not ArgumentType.WSTRING:
            continue
...
def _int_instances(sf, spec, scope=None):
    ...
    for variable in _variables_in_scope(spec, scope):
        ...
        if (
            variable.type.type is ArgumentType.ENUM
            and variable.type.definition.fq_cpp_name is not None
        ):
            continue
...
def _unsigned_long_instances(sf, spec, scope=None):
    ...
    return _write_int_instances(sf, spec, scope, ArgumentType.ULONG, "unsigned long")
...
def _long_long_instances(sf, spec, scope=None):
    ...
    return _write_int_instances(sf, spec, scope, ArgumentType.LONGLONG, "long long")
...
def _unsigned_long_long_instances(sf, spec, scope=None):
    ...
    return _write_int_instances(
        sf, spec, scope, ArgumentType.ULONGLONG, "unsigned long long"
    )
...
def _write_int_instances(sf, spec, scope, target_type, type_name):
    ...
    for variable in _variables_in_scope(spec, scope):
        ...
        if (
            variable_type in (ArgumentType.UINT, ArgumentType.SIZE)
            and target_type is ArgumentType.ULONG
        ):
...
def _empty_iface_file(spec, iface_file):
    ...
    for klass in spec.classes:
        if (
            ...
            and not klass.is_hidden_namespace
            and not klass.is_protected
            and not klass.external
        ):
            return False
...
def _iface_file_cpp(
    ...
    if sf is None:
        source_name = os.path.join(
            buildable.build_dir, "sip" + iface_file.module.py_name
        )
...
def _mapped_type_cpp(sf, spec, bindings, mapped_type):
    ...
    if not mapped_type.no_release:
        ...
        if not mapped_type.no_assignment_operator:
            ...
            if not spec.c_bindings:
                sf.write(
                    f'extern "C" {{static void assign_{mapped_type_name}(void *, Py_ssize_t, void *);}}\n'
                )
            ...
            if spec.c_bindings:
                sf.write(
                    f"    (({mapped_type_type} *)sipDst)[sipDstIdx] = *(({mapped_type_type} *)sipSrc);\n"
                )
            else:
                sf.write(
                    f"    reinterpret_cast<{mapped_type_type} *>(sipDst)[sipDstIdx] = *reinterpret_cast<{mapped_type_type} *>(sipSrc);\n"
                )
        ...
        if not mapped_type.no_default_ctor:
            ...
            if not spec.c_bindings:
                sf.write(
                    f'extern "C" {{static void *array_{mapped_type_name}(Py_ssize_t);}}\n'
                )

            sf.write(
                f"static void *array_{mapped_type_name}(Py_ssize_t sipNrElem)\n{{\n"
            )

            if spec.c_bindings:
                sf.write(
                    f"    return sipMalloc(sizeof ({mapped_type_type}) * sipNrElem);\n"
                )
            else:
        ...
        if not mapped_type.no_copy_ctor:
            ...
            if not spec.c_bindings:
                sf.write(
                    f'extern "C" {{static void *copy_{mapped_type_name}(const void *, Py_ssize_t);}}\n'
                )

            sf.write(
                f"static void *copy_{mapped_type_name}(const void *sipSrc, Py_ssize_t sipSrcIdx)\n{{\n"
            )
...
"""
                ...
                sf.write(
                    f"    return new {mapped_type_type}(reinterpret_cast<const {mapped_type_type} *>(sipSrc)[sipSrcIdx]);\n"
                )
        ...
        if not spec.c_bindings:
            ...
            sf.write(
                f'extern "C" {{static void release_{mapped_type_name}(void *, int{arg_3});}}\n'
            )
    ...
    if mapped_type.convert_from_type_code is not None:
        ...
        if not spec.c_bindings:
            sf.write(
                f'extern "C" {{static PyObject *convertFrom_{mapped_type_name}(void *, PyObject *);}}\n'
            )

        xfer = _use_in_code(
            mapped_type.convert_from_type_code, "sipTransferObj", spec=spec
        )
...
"""
    ...
    cod_name = (
        _cached_name_ref(mapped_type.py_name, as_nr=True) if needs_namespace else "-1"
    )
...
"""
    ...
    if spec.target_abi < (13, 0):
        cod_enummembers = (
            "SIP_NULLPTR"
            if cod_nrenummembers == 0
            else "enummembers_" + mapped_type_name
        )
...
"""
    ...
    mtd_assign = (
        "SIP_NULLPTR"
        if mapped_type.no_assignment_operator
        else "assign_" + mapped_type_name
    )
    mtd_array = (
        "SIP_NULLPTR" if mapped_type.no_default_ctor else "array_" + mapped_type_name
    )
    mtd_copy = "SIP_NULLPTR" if mapped_type.no_copy_ctor else "copy_" + mapped_type_name
    mtd_release = (
        "SIP_NULLPTR" if mapped_type.no_release else "release_" + mapped_type_name
    )
    mtd_cto = (
        "SIP_NULLPTR"
        if mapped_type.convert_to_type_code is None
        else "convertTo_" + mapped_type_name
    )
    mtd_cfrom = (
        "SIP_NULLPTR"
        if mapped_type.convert_from_type_code is None
        else "convertFrom_" + mapped_type_name
    )
...
def _class_cpp(sf, spec, bindings, klass, py_debug):
    ...
    if klass.iface_file.type is not IfaceFileType.NAMESPACE:
        ...
        if klass.convert_from_type_code is not None:
            ...
            xfer = _use_in_code(
                klass.convert_from_type_code, "sipTransferObj", spec=spec
            )

            if not spec.c_bindings:
                sf.write(
                    f'extern "C" {{static PyObject *convertFrom_{name}(void *, PyObject *);}}\n'
                )
...
def _get_method_table(klass):
    ...
    for visible_member in klass.visible_members:
        ...
        for overload in visible_member.scope.overloads:
            ...
            if (
                overload.access_specifier is AccessSpecifier.PROTECTED
                and not klass.has_shadow
            ):
                continue

            if not _skip_overload(
                overload, visible_member.member, klass, visible_member.scope
            ):
                need_member = True
...
"""
        ...
        sf.write(
            f"    {{{cached_py_name}, {cast}meth_{scope_name}_{py_name.name}{cast_suffix}, METH_VARARGS{flags}, {docstring}}}{comma}\n"
        )
...
def _convert_to_definitions(sf, spec, scope):
    ...
    if not spec.c_bindings:
        sf.write(
            f'extern "C" {{static int convertTo_{scope_name}(PyObject *, void **, int *, PyObject *'
        )
    ...
    sf.write(
        f"static int convertTo_{scope_name}(PyObject *{sip_py}, void **{sip_cpp_ptr_v}, int *{sip_is_err}, PyObject *{xfer}"
    )
    ...
    if sip_cpp_ptr != "":
        type_s = fmt_argument_as_cpp_type(spec, scope_type, plain=True, no_derefs=True)
...
def _variable_getter(sf, spec, variable):
    ...
    needs_new = (
        variable_type in (ArgumentType.CLASS, ArgumentType.MAPPED)
        and len(variable.type.derefs) == 0
        and variable.type.is_const
    )
    ...
    if (
        variable_type is ArgumentType.CLASS
        and len(variable.type.derefs) == 0
    ...
    if not spec.c_bindings:
        sf.write(
            f'extern "C" {{static PyObject *varget_{variable_as_word}(void *, PyObject *, PyObject *);}}\n'
        )
...
"""
    ...
    if variable.get_code is None:
        value_decl = _get_named_value_decl(
            spec, variable.scope, variable.type, "sipVal"
        )
...
"""
    ...
    variable_type_s = fmt_argument_as_cpp_type(
        spec, variable.type, plain=True, no_derefs=True
    )
    ...
    if variable_type in (ArgumentType.CLASS, ArgumentType.MAPPED):
        ...
        sf.write(
            f"    {prefix_s} sipConvertFrom{new_s}Type({sip_val_s}, {_gto_name(variable.type.definition)}, SIP_NULLPTR);\n"
        )
...
"""
        ...
        if variable.type.definition.fq_cpp_name is None:
            ...
            sf.write(
                f"    return sipConvertFromEnum({sip_val_s}, {_gto_name(variable.type.definition)});\n"
            )
        ...
        sf.write(
            f'    return PyCapsule_New({cast_s}sipVal, "{variable.type.definition.as_cpp}", SIP_NULLPTR);\n'
        )
...
def _variable_setter(sf, spec, variable):
    ...
    need_sip_cpp = (
        spec.c_bindings
        or variable.set_code is None
        or _is_used_in_code(variable.set_code, "sipCpp")
    )
    ...
    last_arg = (
        "sipPySelf" if spec.c_bindings or (not variable.is_static and keep) else ""
    )
    ...
    sip_py = (
        ...
        if spec.c_bindings
        or variable.set_code is None
        or _is_used_in_code(variable.set_code, "sipPy")
        else ""
    )
    variable_as_word = variable.fq_cpp_name.as_word
    ...
    if not spec.c_bindings:
        sf.write(
            f'extern "C" {{static int varset_{variable_as_word}(void *, PyObject *, PyObject *);}}\n'
        )
...
"""
    ...
    if variable_type in (ArgumentType.CLASS, ArgumentType.MAPPED):
        ...
        if len(variable.type.derefs) == 0:
            ...
            if (
                variable_type is ArgumentType.MAPPED
                and variable.type.definition.no_release
            ):
...
def _variable_to_cpp(spec, variable, has_state):
    ...
    type_s = fmt_argument_as_cpp_type(spec, variable.type, plain=True, no_derefs=True)
    ...
    if variable_type in (ArgumentType.CLASS, ArgumentType.MAPPED):
        ...
        statement = (
            f"({type_s})sipConvertToEnum(sipPy, {_gto_name(variable.type.definition)})"
        )
...
"""
        ...
        for overload in overloads:
            if overload.common is member:
                ...
                _function_body(
                    sf, spec, bindings, scope, overload, dereferenced=dereferenced
                )

        if has_args:
            if member.py_slot in (
                PySlot.CONCAT,
                PySlot.ICONCAT,
                PySlot.REPEAT,
                PySlot.IREPEAT,
...
"""
                ...
                elif is_number_slot(member.py_slot) or is_inplace_number_slot(
                    member.py_slot
                ):
                    sf.write(
                        """
...
"""
                ...
                if is_number_slot(member.py_slot) or is_rich_compare_slot(
                    member.py_slot
                ):
...
"""
                    ...
                    member_name = (
                        "(sipValue != SIP_NULLPTR ? sipName___setattr__ : sipName___delattr__)"
                        if member.py_slot is PySlot.SETATTR
                        else _cached_name_ref(member.py_name)
                    )
...
def _class_functions(sf, spec, bindings, klass, py_debug):
    ...
    for visible_member in klass.visible_members:
        if visible_member.member.py_slot is None:
            _member_function(
                sf, spec, bindings, klass, visible_member.member, visible_member.scope
            )
...
"""
    ...
    if klass.iface_file.type is not IfaceFileType.NAMESPACE and not spec.c_bindings:
        ...
        need_state = False
        need_ptr = need_cast_ptr = _is_used_in_code(klass.dealloc_code, "sipCpp")
        ...
        sf.write(
            f"static void release_{as_word}(void *{sip_cpp_v}, int{sip_state})\n{{\n"
        )
...
"""
    ...
    if klass.gc_traverse_code is not None:
        ...
        if not spec.c_bindings:
            sf.write(
                f'extern "C" {{static int traverse_{as_word}(void *, visitproc, void *);}}\n'
            )
...
"""
    ...
    if klass.bi_get_buffer_code is not None:
        ...
        if not py_debug and spec.module.use_limited_api:
            if not spec.c_bindings:
                sf.write(
                    f'extern "C" {{static int getbuffer_{as_word}(PyObject *, void *, sipBufferDef *);}}\n'
                )

            sf.write(
                f"static int getbuffer_{as_word}(PyObject *{sip_self}, void *{sip_cpp_v}, sipBufferDef *sipBuffer)\n"
            )
        else:
            if not spec.c_bindings:
                sf.write(
                    f'extern "C" {{static int getbuffer_{as_word}(PyObject *, void *, Py_buffer *, int);}}\n'
                )

            sip_flags = _arg_name(spec, "sipFlags", code)

            sf.write(
                f"static int getbuffer_{as_word}(PyObject *{sip_self}, void *{sip_cpp_v}, Py_buffer *sipBuffer, int {sip_flags})\n"
            )
    ...
    if klass.bi_release_buffer_code is not None:
        ...
        if not py_debug and spec.module.use_limited_api:
            if not spec.c_bindings:
                sf.write(
                    f'extern "C" {{static void releasebuffer_{as_word}(PyObject *, void *);}}\n'
                )

            sf.write(
                f"static void releasebuffer_{as_word}(PyObject *{sip_self}, void *{sip_cpp_v})\n"
            )
        else:
            if not spec.c_bindings:
                sf.write(
                    f'extern "C" {{static void releasebuffer_{as_word}(PyObject *, void *, Py_buffer *);}}\n'
                )

            sip_buffer = _arg_name(spec, "sipBuffer", code)

            sf.write(
                f"static void releasebuffer_{as_word}(PyObject *{sip_self}, void *{sip_cpp_v}, Py_buffer *{sip_buffer})\n"
            )
...
"""
    ...
    if klass.finalisation_code is not None:
        ...
        if not spec.c_bindings:
            sf.write(
                f'extern "C" {{static int final_{as_word}(PyObject *, void *, PyObject *, PyObject **);}}\n'
            )
...
"""
    ...
    if klass.mixin:
        ...
        if not spec.c_bindings:
            sf.write(
                f'extern "C" {{static int mixin_{as_word}(PyObject *, PyObject *, PyObject *);}}\n'
            )
...
"""
    ...
    if spec.c_bindings or klass.needs_array_helper:
        ...
        if _abi_supports_array(spec):
            ...
            if not spec.c_bindings:
                sf.write(
                    f'extern "C" {{static void array_delete_{as_word}(void *);}}\n'
                )
    ...
    if spec.c_bindings or klass.needs_copy_helper:
        ...
        if not spec.c_bindings:
            sf.write(
                f'extern "C" {{static void assign_{as_word}(void *, Py_ssize_t, void *);}}\n'
            )
        ...
        if spec.c_bindings:
            sf.write(
                f"    (({scope_s} *)sipDst)[sipDstIdx] = *(({scope_s} *)sipSrc);\n"
            )
        else:
            sf.write(
                f"    reinterpret_cast<{scope_s} *>(sipDst)[sipDstIdx] = *reinterpret_cast<{scope_s} *>(sipSrc);\n"
            )
        ...
        if not spec.c_bindings:
            sf.write(
                f'extern "C" {{static void *copy_{as_word}(const void *, Py_ssize_t);}}\n'
            )

        sf.write(
            f"static void *copy_{as_word}(const void *sipSrc, Py_ssize_t sipSrcIdx)\n{{\n"
        )
...
"""
    ...
    if _need_dealloc(spec, bindings, klass):
        ...
        if not spec.c_bindings:
            sf.write(
                f'extern "C" {{static void dealloc_{as_word}(sipSimpleWrapper *);}}\n'
            )
...
"""
        ...
        if (
            spec.c_bindings
            or klass.dtor is AccessSpecifier.PUBLIC
            or (klass.has_shadow and klass.dtor is AccessSpecifier.PROTECTED)
        ):
            ...
            if klass.delay_dtor:
                ...
                sf.write(
                    f"        release_{as_word}(sipGetAddress(sipSelf), {flag});\n"
                )
...
def _shadow_code(sf, spec, bindings, klass):
    ...
    for ctor in _unique_class_ctors(spec, klass):
        ...
        args = fmt_signature_as_cpp_definition(
            spec, ctor.cpp_signature, scope=klass.iface_file
        )
        ...
        if bindings.tracing:
            args = fmt_signature_as_cpp_declaration(
                spec, ctor.cpp_signature, scope=klass.iface_file
            )

            sf.write(
                f'    sipTrace(SIP_TRACE_CTORS, "sip{klass_name}::sip{klass_name}({args}){throw_specifier} (this=0x%08x)\\n", this);\n\n'
            )
    ...
    if klass.dtor is not AccessSpecifier.PRIVATE:
        ...
        if bindings.tracing:
            sf.write(
                f'    sipTrace(SIP_TRACE_DTORS, "sip{klass_name}::~sip{klass_name}(){throw_specifier} (this=0x%08x)\\n", this);\n\n'
            )
...
"""
    ...
    for virt_nr, virtual_overload in enumerate(
        _unique_class_virtual_overloads(spec, klass)
    ):
...
def _protected_enums(sf, spec, klass):
    ...
    enum"""
        ...
        for member in enum.members:
            ...
            sf.write(
                f"{eol}        {member_cpp_name} = {scope_cpp_name}::{member_cpp_name}"
            )
...
def _virtual_catcher(sf, spec, bindings, klass, virtual_overload, virt_nr):
    ...
    result_type = fmt_argument_as_cpp_type(
        spec, result, scope=klass.iface_file, make_public=True
    )
    klass_name = klass.iface_file.fq_cpp_name.as_word
    ...
    args = fmt_signature_as_cpp_definition(
        spec, overload.cpp_signature, scope=klass.iface_file
    )
    sf.write(
    ...
    if bindings.tracing:
        ...
        sf.write(
            f'    sipTrace(SIP_TRACE_CATCHERS, "{result_type} sip{klass_name}::{overload_cpp_name}({args}){const}{throw_specifier} (this=0x%08x)\\n", this);\n\n'
        )
...
"""
    ...
    abi_12_8_arg = (
        f"{const_cast_sw}&sipPySelf{const_cast_tail}, "
        if spec.target_abi >= (12, 8)
        else ""
    )

    klass_py_name_ref = (
        _cached_name_ref(klass.py_name) if overload.is_abstract else "SIP_NULLPTR"
    )
    member_py_name_ref = _cached_name_ref(overload.common.py_name)

    sf.write(
        f"    sipMeth = sipIsPyMethod(&sipGILState, {const_cast_char}&sipPyMethods[{virt_nr}]{const_cast_tail}, {abi_12_8_arg}{klass_py_name_ref}, {member_py_name_ref});\n"
    )
    ...
    if (
        result is not None
        and result.type is ArgumentType.VOID
        and len(result.derefs) == 0
    ):
        result = None
    ...
    if overload.virtual_call_code is not None:
        ...
        if result is not None:
            sip_res = fmt_argument_as_cpp_type(
                spec, result, name="sipRes", scope=klass.iface_file
            )
            sf.write(f"        {sip_res};\n")
...
"""
        ...
        sf.write(
            f"{klass.iface_file.fq_cpp_name.as_cpp}::{overload_cpp_name}({args});\n"
        )
...
def _virtual_handler_call(sf, spec, klass, virtual_overload, result):
    ...
    result_type = fmt_argument_as_cpp_type(
        spec, overload.cpp_signature.result, scope=klass.iface_file
    )

    sf.write(
        f"    extern {result_type} sipVH_{module_name}_{handler.handler_nr}(sip_gilstate_t, sipVirtErrorHandlerFunc, sipSimpleWrapper *, PyObject *"
    )
    ...
    if len(handler.cpp_signature.args) > 0:
        sf.write(
            ", "
            + fmt_signature_as_cpp_declaration(
                spec, handler.cpp_signature, scope=klass.iface_file
            )
        )
    ...
    sf.write(
        f"sipVH_{module_name}_{handler.handler_nr}(sipGILState, {error_handler_ref}, sipPySelf, sipMeth"
    )
...
def _default_instance_return(sf, spec, result):
    ...
    result_type_plain = fmt_argument_as_cpp_type(
        spec, result, plain=True, no_derefs=True
    )

    # Handle any %InstanceCode.
    if len(result.derefs) == 0 and result.type in (
        ArgumentType.CLASS,
        ArgumentType.MAPPED,
    ):
...
"""
    ...
    if result.type is ArgumentType.MAPPED and len(result.derefs) == 0:
        ...
        if (
            ctor is not None
            and ctor.access_specifier is AccessSpecifier.PUBLIC
            and ctor.cpp_signature is not None
        ):
            ...
            raise UserException(
                result.definition.iface_file.fq_cpp_name.as_cpp
...
def _call_default_ctor(spec, ctor):
    ...
    for arg in ctor.cpp_signature.args:
        ...
        if (
            arg.type is ArgumentType.CLASS
            and len(arg.derefs) > 0
            and not arg.is_reference
        ):
...
def _protected_declarations(sf, spec, klass):
    ...
    for visible_member in klass.visible_members:
        ...
        for overload in visible_member.scope.overloads:
            if (
                overload.common is not visible_member.member
                or overload.access_specifier is not AccessSpecifier.PROTECTED
            ):
                continue
...
"""
            ...
            if (
                not overload.is_static
                and not overload.is_abstract
                and (overload.is_virtual or overload.is_virtual_reimplementation)
            ):
...
def _protected_definitions(sf, spec, klass):
    ...
    for visible_member in klass.visible_members:
        ...
        for overload in visible_member.scope.overloads:
            ...
            result = overload.cpp_signature.result
            result_type = fmt_argument_as_cpp_type(spec, result, scope=klass.iface_file)
            ...
            if (
                ...
                sf.write(
                    f"{result_type} sip{klass_name}::sipProtectVirt_{overload_name}(bool sipSelfWasArg"
                )
            ...
            if result.type is ArgumentType.VOID and len(result.derefs) == 0:
                ...
                if result.type is ArgumentType.CLASS and result.definition.is_protected:
                    ...
                    result.type is ArgumentType.ENUM and result.definition.is_protected
                ):
            ...
            protected_call_args = _protected_call_args(spec, overload.cpp_signature)
            ...
            if not overload.is_abstract:
                ...
                if overload.is_virtual or overload.is_virtual_reimplementation:
                    sf.write(
                        f"(sipSelfWasArg ? {visible_scope_s}::{overload_name}({protected_call_args}) : "
                    )
                    closing_parens += ")"
                else:
...
def _is_duplicate_protected(spec, klass, target_overload):
    ...
    for visible_member in klass.visible_members:
        ...
        for overload in visible_member.scope.overloads:
            ...
            if overload.cpp_name == target_overload.cpp_name and same_signature(
                spec, overload.cpp_signature, target_overload.cpp_signature
            ):
                return True
...
def _virtual_handler(sf, spec, handler):
    ...
    if result_is_returned:
        ...
        if (
            result.type in (ArgumentType.CLASS, ArgumentType.MAPPED)
            and len(result.derefs) == 0
        ):
            if result.is_reference:
...
{result_decl} sipVH_{module.py_name}_{handler.handler_nr}(sip_gilstate_t sipGILState, sipVirtErrorHandlerFunc sipErrorHandler, sipSimpleWrapper *sipPySelf, PyObject *sipMethod"""
    ...
    if len(handler.cpp_signature.args) > 0:
        sf.write(", " + fmt_signature_as_cpp_definition(spec, handler.cpp_signature))

    # Define the extra arguments for kept references.
    if result_is_returned and _keep_py_reference(result):
        sf.write(", int")

        if handler.virtual_catcher_code is None or _is_used_in_code(
            handler.virtual_catcher_code, "sipResKey"
        ):
            sf.write(" sipResKey")
...
"""
        ...
        if (
            result.type
            in (ArgumentType.CLASS, ArgumentType.MAPPED, ArgumentType.TEMPLATE)
            and len(result.derefs) == 0
        ):
            if result_instance_code is not None:
                sipres_value = " = *sipCpp"
                ...
                if (
                    ...
                    and ctor.access_specifier is AccessSpecifier.PUBLIC
                    and ctor.cpp_signature is not None
                    and len(ctor.cpp_signature.args) > 0
                    and ctor.cpp_signature.args[0].default_value is None
                ):
...
"""
    ...
    if nr_values == 0:
        ...
        sf.write("    PyObject *sipResObj = sipCallMethod(SIP_NULLPTR, sipMethod, ")
...
"""
    ...
    params = ["sipGILState", "sipErrorHandler", "sipPySelf", "sipMethod", "sipResObj"]
    ...
    return_code = (
        "int sipRc = " if result_is_reference or handler.abort_on_exception else ""
    )
...
def _get_parse_result_format(
    arg, spec, result_is_reference=False, transfer_result=False
):
    ...
    for arg_nr, arg in enumerate(signature.args):
        ...
        if arg.type in (
            ...
            if needs_copy:
                prefix = (
                    "new "
                    + fmt_argument_as_cpp_type(spec, arg, plain=True, no_derefs=True)
                    + "("
                )
            else:
                if arg.is_const:
                    prefix = (
                        "const_cast<"
                        + fmt_argument_as_cpp_type(
                            spec, arg, plain=True, no_derefs=True, use_typename=False
                        )
                        + " *>("
                    )
            ...
            if arg.array is ArrayArgument.ARRAY:
                array_len_arg_name = fmt_argument_as_name(
                    spec, signature.args[array_len_arg_nr], array_len_arg_nr
                )
            ...
            if arg.array is not ArrayArgument.ARRAY_SIZE:
                args.append("*" * nr_derefs + fmt_argument_as_name(spec, arg, arg_nr))
            ...
            if arg.array is ArrayArgument.ARRAY:
                ...
                arg.type is ArgumentType.ENUM and arg.definition.fq_cpp_name is not None
            ):
                args.append(_gto_name(arg.definition))
...
"""
            ...
            sf.write(
                f"#define sipException_{exception.iface_file.fq_cpp_name.as_word} sipExportedExceptions_{module_name}[{exception.exception_nr}]\n"
            )
...
def _imported_module_api(sf, spec, imported_module):
    ...
    for klass in spec.classes:
        ...
        if iface_file.module is imported_module:
            ...
            _enum_macros(sf, spec, scope=klass, imported_module=imported_module)
    ...
    for mapped_type in spec.mapped_types:
        ...
        if iface_file.module is imported_module:
            ...
            _enum_macros(sf, spec, scope=mapped_type, imported_module=imported_module)
...
def _class_api(sf, spec, klass):
    ...
    if klass.real_class is None and not klass.is_hidden_namespace:
        sf.write(
            f"#define {_gto_name(klass)} sipExportedTypes_{module_name}[{iface_file.type_nr}]\n"
        )
...
def _overload_decl(sf, spec, bindings, klass, overload):
    ...
    result_type = fmt_argument_as_cpp_type(
        spec, cpp_signature.result, scope=klass.iface_file
    )

    args = []
    for arg in cpp_signature.args:
        args.append(fmt_argument_as_cpp_type(spec, arg, scope=klass.iface_file))
    ...
    sf.write(
        f"{result_type} {_overload_cpp_name(overload)}({args}){const_s}{throw_specifier} SIP_OVERRIDE"
    )
...
def _call_args(sf, spec, cpp_signature, py_signature):
    ...
    for arg_nr, arg in enumerate(cpp_signature.args):
        ...
        if py_signature is not cpp_signature and len(py_signature.args) == len(
            cpp_signature.args
        ):
            ...
            if (
                py_arg.type in VOID_TYPES
                and arg.type not in VOID_TYPES
                and len(py_arg.derefs) == nr_derefs
            ):
                need_cast = True

        arg_name = fmt_argument_as_name(spec, arg, arg_nr)
        arg_cpp_type_name = fmt_argument_as_cpp_type(
            spec, arg, plain=True, no_derefs=True
        )
...
def _get_named_value_decl(spec, scope, type, name):
    ...
    named_value_decl = fmt_argument_as_cpp_type(
        spec,
        type,
        name=name,
        scope=(
            scope.iface_file if isinstance(scope, (WrappedClass, MappedType)) else None
        ),
    )
...
def _argument_variable(sf, spec, scope, arg, arg_nr):
    ...
    if (
        ...
        and arg.type in (ArgumentType.CLASS, ArgumentType.MAPPED)
        and (nr_derefs == 0 or arg.is_reference)
    ):
        arg_cpp_type = fmt_argument_as_cpp_type(spec, arg, scope=scope_iface_file)
    ...
    modified_arg_cpp_type = fmt_argument_as_cpp_type(
        spec, arg, scope=scope_iface_file, use_typename=use_typename
    )
    ...
    if arg.is_in and arg.default_value is not None:
        ...
        if arg.type in (ArgumentType.CLASS, ArgumentType.MAPPED) and (
            nr_derefs == 0 or arg.is_reference
        ):
    ...
    if arg.is_in:
        if arg.get_wrapper:
            sf.write(
                f"        PyObject *{arg_name}Wrapper{supporting_default_value};\n"
            )
        ...
        if arg.type is ArgumentType.CLASS:
            if arg.array is ArrayArgument.ARRAY and _abi_supports_array(spec):
                ...
                if (
                    arg.definition.convert_to_type_code is not None
                    and not arg.is_constrained
                ):
            ...
            if arg.key is None and nr_derefs == 1:
                sf.write(
                    f"        PyObject *{arg_name}Keep{supporting_default_value};\n"
                )
...
"""
    ...
    if klass.has_variable_handlers:
        for variable in spec.variables:
            if variable.scope is klass and variable.needs_handler:
                ...
                fields.append(
                    "ClassVariable" if variable.is_static else "InstanceVariable"
                )
    ...
    container_fields.append(
        _cached_name_ref(klass.py_name, as_nr=True)
    ...
    instances.append(_class_object_ref(is_inst_class, "typeInstances", klass_name))
    instances.append(_class_object_ref(is_inst_voidp, "voidPtrInstances", klass_name))
    instances.append(_class_object_ref(is_inst_char, "charInstances", klass_name))
    instances.append(_class_object_ref(is_inst_string, "stringInstances", klass_name))
    instances.append(_class_object_ref(is_inst_int, "intInstances", klass_name))
    instances.append(_class_object_ref(is_inst_long, "longInstances", klass_name))
    instances.append(
        _class_object_ref(is_inst_ulong, "unsignedLongInstances", klass_name)
    )
    instances.append(
        _class_object_ref(is_inst_longlong, "longLongInstances", klass_name)
    )
    instances.append(
        _class_object_ref(is_inst_ulonglong, "unsignedLongLongInstances", klass_name)
    )
    instances.append(_class_object_ref(is_inst_double, "doubleInstances", klass_name))

    container_fields.append("{" + ", ".join(instances) + "}")

    class_fields.append(docstring_ref)
    class_fields.append(
        _cached_name_ref(klass.metatype, as_nr=True)
        if klass.metatype is not None
        else "-1"
    )
    class_fields.append(
        _cached_name_ref(klass.supertype, as_nr=True)
        if klass.supertype is not None
        else "-1"
    )
    class_fields.append(
        _class_object_ref((len(klass.superclasses) != 0), "supers", klass_name)
    )
    class_fields.append(_class_object_ref(is_slots, "slots", klass_name))
    class_fields.append(_class_object_ref(klass.can_create, "init_type", klass_name))
    class_fields.append(
        _class_object_ref((klass.gc_traverse_code is not None), "traverse", klass_name)
    )
    class_fields.append(
        _class_object_ref((klass.gc_clear_code is not None), "clear", klass_name)
    )
    class_fields.append(
        _class_object_ref(
            (klass.bi_get_buffer_code is not None), "getbuffer", klass_name
        )
    )
    class_fields.append(
        _class_object_ref(
            (klass.bi_release_buffer_code is not None), "releasebuffer", klass_name
        )
    )
    class_fields.append(
        _class_object_ref(_need_dealloc(spec, bindings, klass), "dealloc", klass_name)
    )
    class_fields.append(
        _class_object_ref(
            (spec.c_bindings or klass.needs_copy_helper), "assign", klass_name
        )
    )
    class_fields.append(
        _class_object_ref(
            (spec.c_bindings or klass.needs_array_helper), "array", klass_name
        )
    )
    class_fields.append(
        _class_object_ref(
            (spec.c_bindings or klass.needs_copy_helper), "copy", klass_name
        )
    )
    class_fields.append(
        _class_object_ref(
            (
                not spec.c_bindings
                and klass.iface_file.type is not IfaceFileType.NAMESPACE
            ),
            "release",
            klass_name,
        )
    ...
    class_fields.append(
        _class_object_ref((len(klass.superclasses) != 0), "cast", klass_name)
    )
    class_fields.append(
        _class_object_ref(
            (
                klass.convert_to_type_code is not None
                and klass.iface_file.type is not IfaceFileType.NAMESPACE
            ),
            "convertTo",
            klass_name,
    ...
    class_fields.append(
        _class_object_ref(
            (
                klass.convert_from_type_code is not None
                and klass.iface_file.type is not IfaceFileType.NAMESPACE
            ),
            "convertFrom",
            klass_name,
    ...
    class_fields.append(
        _class_object_ref((klass.pickle_code is not None), "pickle", klass_name)
    )
    class_fields.append(
        _class_object_ref((klass.finalisation_code is not None), "final", klass_name)
    )
...
def _pyqt_emitters(sf, spec, klass):
    ...
    for member in klass.members:
        ...
        for overload in klass.overloads:
            if not (
                overload.common is member
                and overload.pyqt_method_specifier is PyQtMethodSpecifier.SIGNAL
                and _has_optional_args(overload)
            ):
                continue
            ...
            if not in_emitter:
                ...
                if not spec.c_bindings:
                    sf.write(
                        f'extern "C" {{static int emit_{klass_name}_{overload.cpp_name}(void *, PyObject *);}}\n\n'
                    )
...
def _pyqt_signal_table_entry(sf, spec, bindings, klass, signal, member_nr):
    ...
    for arg in signal.cpp_signature.args:
        ...
        args.append(
            fmt_argument_as_cpp_type(spec, arg, scope=klass.iface_file, strip=strip)
        )
    ...
    if stripped:
        ...
        for arg in signal.cpp_signature.args:
            args.append(
                fmt_argument_as_cpp_type(
                    spec, arg, scope=klass.iface_file, strip=STRIP_GLOBAL
                )
            )
    ...
    sf.write(
        f"&methods_{klass_name}[{member_nr}], " if member_nr >= 0 else "SIP_NULLPTR, "
    )

    sf.write(
        f"emit_{klass_name}_{signal.cpp_name}"
...
def _type_init(sf, spec, bindings, klass):
    ...
    if not spec.c_bindings:
        sf.write(
            f'extern "C" {{static void *init_type_{klass_name}(sipSimpleWrapper *, PyObject *, PyObject *, PyObject **, PyObject **, PyObject **);}}\n'
        )

    sip_self = "sipSelf" if need_self else ""
    sip_owner = "sipOwner" if need_owner else ""
    sip_cpp_type = (
        "sip" + klass_name if klass.has_shadow else _scoped_class_name(spec, klass)
    )
...
"""
    ...
    for ctor in klass.ctors:
        ...
        _constructor_call(sf, spec, bindings, klass, ctor, error_flag, old_error_flag)
...
def _handling_exceptions(bindings, throw_args):
    ...
    # throw specifier.
    return bindings.exceptions and (
        throw_args is None or throw_args.arguments is not None
    )
...
def _catch_block(sf, spec, exception, py_signature=None, release_gil=False):
    ...
    sip_exception_ref = (
        ...
        or _is_used_in_code(exception.raise_code, "sipExceptionRef")
        else ""
    )
...
def _throw_specifier(bindings, throw_args):
    ...
    return (
        " noexcept"
        if bindings.exceptions
        and throw_args is not None
        and throw_args.arguments is None
        else ""
    )


def _constructor_call(sf, spec, bindings, klass, ctor, error_flag, old_error_flag):
    ...
    if ctor.deprecated is not None:
        ...
        if _abi_has_deprecated_message(spec):
            str_deprecated_message = (
                f'"{ctor.deprecated}"' if ctor.deprecated else "SIP_NULLPTR"
            )
            sf.write(
    ...
    if ctor.method_code is not None:
        ...
        _catch(sf, spec, bindings, ctor.py_signature, ctor.throw_args, release_gil)
        ...
        if arg.key is not None:
            ...
            suffix = (
                ...
                if (
                    ...
                    and len(arg.derefs) == 1
                )
                or not arg.get_wrapper
                else "Wrapper"
            )
...
def _skip_overload(overload, member, klass, scope, want_local=True):
    ...
    if (
        want_local
        and overload.access_specifier is not AccessSpecifier.PROTECTED
        and klass is not scope
    ):
        return True
...
def _member_function(sf, spec, bindings, klass, member, original_klass):
            ...
            if overload.access_specifier is not AccessSpecifier.PRIVATE:
                ...
                if spec.target_abi >= (13, 0) or not overload.is_static:
                    ...
                    if overload.is_abstract:
                        ...
                        overload.is_virtual
                        or overload.is_virtual_reimplementation
                        or _is_used_in_code(overload.method_code, "sipSelfWasArg")
                    ):
                        need_selfarg = True
    ...
    if _has_member_docstring(bindings, member, original_klass.overloads):
        ...
        has_auto_docstring = _member_docstring(
            sf,
            spec,
            bindings,
            member,
    ...
    if not spec.c_bindings:
        sf.write(
            f'extern "C" {{static PyObject *meth_{klass_name}_{member_py_name}(PyObject *, PyObject *{arg3_type});}}\n'
        )

    sf.write(
        f"static PyObject *meth_{klass_name}_{member_py_name}(PyObject *{sip_self}, PyObject *{sip_args}{arg3_decl})\n{{\n"
    )

    if bindings.tracing:
        sf.write(
            f'    sipTrace(SIP_TRACE_METHODS, "meth_{klass_name}_{member_py_name}()\\n");\n\n'
        )
    ...
    if not member.no_arg_parser:
        ...
        if need_selfarg:
            ...
            sf.write(
                f"    bool sipSelfWasArg = ({sipself_test} || sipIsDerivedClass((sipSimpleWrapper *)sipSelf));\n"
            )
    ...
    for overload in original_klass.overloads:
        ...
        _function_body(
            sf, spec, bindings, klass, overload, original_klass=original_klass
        )
    ...
    if not member.no_arg_parser:
        ...
        docstring_ref = (
            f"doc_{klass_name}_{member_py_name}"
            if has_auto_docstring
            else "SIP_NULLPTR"
        )
...
def _function_body(
    sf, spec, bindings, scope, overload, original_klass=None, dereferenced=True
):
    ...
    if is_number_slot(overload.common.py_slot):
        ...
        if len(py_signature.args) == 1:
            ...
            py_signature.args[0] = Argument(
                ArgumentType.CLASS,
                is_in=True,
                is_reference=True,
                definition=original_klass,
            )

            py_signature_adjusted = True

        _arg_parser(sf, spec, scope, py_signature, overload=overload)
    elif not is_int_arg_slot(overload.common.py_slot) and not is_zero_arg_slot(
        overload.common.py_slot
    ):
        _arg_parser(sf, spec, scope, py_signature, overload=overload)

    _function_call(sf, spec, bindings, scope, overload, dereferenced, original_scope)
...
def _handle_result(sf, spec, overload, is_new_instance, result_size_arg_nr, action):
...
"""
    ...
    if result is not None and result.type in (ArgumentType.CLASS, ArgumentType.MAPPED):
        ...
        if is_new_instance or overload.factory:
            ...
            owner = (
                "(PyObject *)sipOwner"
                if has_owner and overload.factory
                else result_owner
            )

            sf.write(
                f"            {this_action} sipConvertFromNewType({sip_res}, {result_gto_name}, {owner});\n"
            )
            ...
            this_action = (
                "PyObject *sipResObj =" if nr_return_values > 1 or need_xfer else action
            )
            owner = "SIP_NULLPTR" if need_xfer else result_owner

            sf.write(
                f"            {this_action} sipConvertFromType({sip_res}, {result_gto_name}, {owner});\n"
            )
    ...
    if nr_return_values > 1:
        ...
        if result is not None:
            format_s += (
                "R"
                if result.type in (ArgumentType.CLASS, ArgumentType.MAPPED)
                else _get_build_result_format(result)
            )
        ...
        if result is not None:
            ...
            if (
                result.type is ArgumentType.ENUM
                and result.definition.fq_cpp_name is not None
            ):
                build_result_args.append(_gto_name(result.definition))

        for arg_nr, arg in enumerate(overload.py_signature.args):
            if arg.is_out:
                build_result_args.append(fmt_argument_as_name(spec, arg, arg_nr))
                ...
                if arg.type in (ArgumentType.CLASS, ArgumentType.MAPPED):
                    ...
                    arg.type is ArgumentType.ENUM
                    and arg.definition.fq_cpp_name is not None
                ):
                    build_result_args.append(_gto_name(arg.definition))
    ...
    if value.type in (ArgumentType.CLASS, ArgumentType.MAPPED):
        ...
        convertor = (
            "sipConvertFromNewType" if need_new_instance else "sipConvertFromType"
        )
        value_name = _const_cast(spec, value, value_name)
        transfer = (
            "Py_None"
            if not need_new_instance and value.transfer is Transfer.TRANSFER_BACK
            else "SIP_NULLPTR"
        )

        sf.write(
            f"            {action} {convertor}({value_name}, {_gto_name(value.definition)}, {transfer});\n"
        )
        ...
        if value.definition.fq_cpp_name is not None:
            ...
            sf.write(
                f"            {action} sipConvertFromEnum({value_name}, {_gto_name(value.definition)});\n"
            )
        else:
        ...
        if len(value.derefs) == 0:
            sf.write(
                f"            {action} PyUnicode_DecodeASCII(&{value_name}, 1, SIP_NULLPTR);\n"
            )
        else:
            sf.write(
...
"""
        ...
        if len(value.derefs) == 0:
            sf.write(
                f"            {action} PyUnicode_DecodeLatin1(&{value_name}, 1, SIP_NULLPTR);\n"
            )
        else:
            sf.write(
                f"""            if ({value_name} == SIP_NULLPTR)
...
"""
        ...
        if len(value.derefs) == 0:
            sf.write(
                f"            {action} PyUnicode_FromStringAndSize(&{value_name}, 1);\n"
            )
        else:
            sf.write(
                f"""            if ({value_name} == SIP_NULLPTR)
...
"""
        ...
        if len(value.derefs) == 0:
            sf.write(
                f"            {action} PyBytes_FromStringAndSize({cast}&{value_name}, 1);\n"
            )
        else:
            sf.write(
                f"""            if ({value_name} == SIP_NULLPTR)
...
"""
        ...
        if len(value.derefs) == 0:
            sf.write(
                f"            {action} PyUnicode_FromWideChar(&{value_name}, 1);\n"
            )
        else:
            sf.write(
                f"""            if ({value_name} == SIP_NULLPTR)
...
"""
        ...
        convertor = (
            "sipConvertFromConstVoidPtr" if value.is_const else "sipConvertFromVoidPtr"
        )
        if result_size_arg_nr >= 0:
            convertor += "AndSize"

        sf.write(
            f"            {action} {convertor}({_get_void_ptr_cast(value)}{value_name}"
        )
        ...
        if result_size_arg_nr >= 0:
            sf.write(
                ...
                + fmt_argument_as_name(
                    spec,
                    overload.py_signature.args[result_size_arg_nr],
                    result_size_arg_nr,
                )
            )
        ...
        sf.write(
            f'            {action} PyCapsule_New({value_name}, "{value.definition.as_cpp}", SIP_NULLPTR);\n'
        )
...
def _function_call(sf, spec, bindings, scope, overload, dereferenced, original_scope):
    """Generate a function call."""

    py_slot = overload.common.py_slot
    result = overload.py_signature.result
    result_cpp_type = fmt_argument_as_cpp_type(spec, result, plain=True, no_derefs=True)
...
"""
    ...
    for arg_nr, arg in enumerate(overload.py_signature.args):
        ...
        if arg.type in (ArgumentType.CLASS, ArgumentType.MAPPED) and _need_new_instance(
            arg
        ):
            arg_name = fmt_argument_as_name(spec, arg, arg_nr)
            arg_cpp_type = fmt_argument_as_cpp_type(
                spec, arg, plain=True, no_derefs=True
            )
...
"""
    ...
    if overload.deprecated is not None:
        scope_py_name_ref = (
            _cached_name_ref(scope.py_name)
            if scope is not None and scope.py_name is not None
            else "SIP_NULLPTR"
        )
        error_return = (
            "-1"
            if is_void_return_slot(py_slot)
            or is_int_return_slot(py_slot)
            or is_ssize_return_slot(py_slot)
            or is_hash_return_slot(py_slot)
            else "SIP_NULLPTR"
        )
        ...
        if _abi_has_deprecated_message(spec):
            str_deprecated_message = (
                f'"{overload.deprecated}"' if overload.deprecated else "SIP_NULLPTR"
            )
            sf.write(
...
"""
        ...
        if result_decl is not None:
            ...
            if is_new_instance:
                if spec.c_bindings:
                    ...
                    result.type is ArgumentType.CLASS and result.definition.cannot_copy
                ):
                    sf.write(
                        f"sipRes = reinterpret_cast<{result_cpp_type} *>(::operator new(sizeof ({result_cpp_type})));\n            *sipRes = "
                    )
                else:
                ...
                # be non-const.
                if result.type in (ArgumentType.CLASS, ArgumentType.MAPPED) and (
                    len(result.derefs) == 0 or result.is_reference
                ):
                    sf.write("&")
        ...
        _catch(
            sf, spec, bindings, overload.py_signature, overload.throw_args, release_gil
        )
    ...
    for arg_nr, arg in enumerate(overload.py_signature.args):
        ...
        if not static_factory and arg.key is not None:
            sip_self = (
                "SIP_NULLPTR" if scope is None or overload.is_static else "sipSelf"
            )
            keep_reference_call = _get_keep_reference_call(spec, arg, arg_nr, sip_self)
...
"""
        ...
        _handle_result(sf, spec, overload, is_new_instance, result_size_arg_nr, action)
        ...
        if static_factory:
            for arg_nr, arg in enumerate(overload.py_signature.args):
                ...
                if arg.key != None:
                    keep_reference_call = _get_keep_reference_call(
                        spec, arg, arg_nr, "sipResObj"
                    )
                    sf.write(f"\n            {keep_reference_call};\n")
...
def _get_result_decl(spec, scope, overload, result):
    ...
    no_result = (
        is_inplace_number_slot(overload.common.py_slot)
        or is_inplace_sequence_slot(overload.common.py_slot)
        or (result.type is ArgumentType.VOID and len(result.derefs) == 0)
    )
    ...
    initial_value = (
        " = " + _cast_zero(spec, result) if overload.method_code is not None else ""
    )
...
def _cpp_function_call(sf, spec, scope, overload, original_scope):
    ...
    if scope is None:
        ...
        if overload.access_specifier is AccessSpecifier.PROTECTED:
            sf.write(
                f"sip{scope.iface_file.fq_cpp_name.as_word}::sipProtect_{cpp_name}("
            )
        else:
        ...
        if not overload.is_abstract and (
            overload.is_virtual or overload.is_virtual_reimplementation
        ):
    ...
    elif not overload.is_abstract and (
        overload.is_virtual or overload.is_virtual_reimplementation
    ):
        sf.write(
            f"(sipSelfWasArg ? sipCpp->{original_scope.iface_file.fq_cpp_name.as_cpp}::{cpp_name}("
        )
        _call_args(sf, spec, overload.cpp_signature, overload.py_signature)
...
def _arg_parser(sf, spec, scope, py_signature, ctor=None, overload=None):
    ...
    if (
        isinstance(scope, WrappedClass)
        and scope.iface_file.type is IfaceFileType.NAMESPACE
    ):
        scope = None
    ...
    if spec.target_abi >= (13, 0):
        handle_self = (
            ...
            and overload.common.py_slot is None
        )
    else:
        handle_self = (
            scope is not None
    ...
    if overload is not None and is_number_slot(overload.common.py_slot):
        ...
        overload is not None and overload.common.allow_keyword_args
    ) or ctor is not None:
        ...
        if kw_args is not KwArgs.NONE:
            for arg in py_signature.args:
                ...
                if arg.name is not None and (
                    kw_args is KwArgs.ALL or arg.default_value is not None
                ):
        ...
        single_arg = not (
            overload is None
            or overload.common.py_slot is None
            or is_multi_arg_slot(overload.common.py_slot)
        )
    ...
    for arg_nr, arg in enumerate(py_signature.args):
        ...
        if arg.type is ArgumentType.MAPPED:
            ...
            if arg.array is ArrayArgument.ARRAY:
                array_len_arg_name = fmt_argument_as_name(
                    spec, py_signature.args[array_len_arg_nr], array_len_arg_nr
                )
                args.append("&" + array_len_arg_name)
            elif (
                mapped_type.convert_to_type_code is not None and not arg.is_constrained
            ):
                args.append(
                    "SIP_NULLPTR" if mapped_type.no_release else f"&{arg_name}State"
                )
...
def _get_subformat_char(arg):
    ...
    if arg.type in (ArgumentType.CLASS, ArgumentType.MAPPED):
        ...
        if arg.is_constrained or (
            arg.type is ArgumentType.CLASS
            and arg.definition.convert_to_type_code is None
        ):
            flags |= 0x08
...
def _get_convert_to_type_code(type):
    ...
    if (
        type.type in (ArgumentType.CLASS, ArgumentType.MAPPED)
        and not type.is_constrained
    ):
...
def _delete_outs(sf, spec, py_signature):
    ...
    for arg_nr, arg in enumerate(py_signature.args):
        if arg.type in (ArgumentType.CLASS, ArgumentType.MAPPED) and _need_new_instance(
            ...
            sf.write(
                f"                delete {fmt_argument_as_name(spec, arg, arg_nr)};\n"
            )
...
def _delete_temporaries(sf, spec, py_signature):
    ...
    for arg_nr, arg in enumerate(py_signature.args):
        ...
        if arg.array is ArrayArgument.ARRAY and arg.type in (
            ArgumentType.CLASS,
            ArgumentType.MAPPED,
        ):
        ...
        if (
            ...
            if convert_to_type_code is not None:
                ...
                if spec.c_bindings or not arg.is_const:
                    ...
                    arg_cpp_plain = fmt_argument_as_cpp_type(
                        spec, arg, plain=True, no_derefs=True
                    )
...
def _remove_protection(arg, protection_state):
    ...
    if (
        arg.type in (ArgumentType.CLASS, ArgumentType.ENUM)
        and arg.definition.is_protected
    ):
...
def _mapped_type_from_void(spec, mapped_type_type):
    ...
    return (
        f"{mapped_type_type} *sipCpp = reinterpret_cast<{mapped_type_type} *>(sipCppV)"
    )
...
def _class_docstring(sf, spec, bindings, klass):
    ...
    if (
        klass.docstring is not None
        and klass.docstring.signature is not DocstringSignature.PREPENDED
    ):
    ...
    if (
        klass.docstring is None
        or klass.docstring.signature is not DocstringSignature.DISCARDED
    ):
        for ctor in klass.ctors:
            if ctor.access_specifier is AccessSpecifier.PRIVATE:
                continue
    ...
    if (
        klass.docstring is not None
        and klass.docstring.signature is DocstringSignature.PREPENDED
    ):
        if not is_first:
            sf.write(NEWLINE)
            sf.write(NEWLINE)
...
def _ctor_auto_docstring(sf, spec, bindings, klass, ctor):
    ...
    if bindings.docstrings:
        ...
        signature = fmt_signature_as_type_hint(
            spec, ctor.py_signature, need_self=False, exclude_result=True
        )
...
def _plugin_signals_table(sf, spec, bindings, klass):
    ...
    for member in klass.members:
        ...
        for overload in klass.overloads:
            if (
                overload.common is not member
                or overload.pyqt_method_specifier is not PyQtMethodSpecifier.SIGNAL
            ):
                continue
            ...
            if member_nr >= 0:
                ...
                for non_sig in klass.overloads:
                    if (
                        non_sig is not overload
                        and non_sig.common is member
                        and non_sig.pyqt_method_specifier
                        is not PyQtMethodSpecifier.SIGNAL
                    ):
                        break
                else:
                    member_nr = -1
...
"""
            ...
            _pyqt_signal_table_entry(sf, spec, bindings, klass, overload, member_nr)
...
def _pyqt_class_plugin(sf, spec, bindings, klass):
    ...
    mo_ref = (
        f"&{_scoped_class_name(spec, klass)}::staticMetaObject"
        if klass.is_qobject and not klass.pyqt_no_qmetaobject
        else "SIP_NULLPTR"
    )
    sf.write(f"    {mo_ref},\n")
...
def _global_function_table_entries(sf, spec, bindings, members):
    ...
    for member in members:
        if member.py_slot is None:
            ...
            if member.no_arg_parser or member.allow_keyword_args:
                sf.write(
                    f"SIP_MLMETH_CAST(func_{member.py_name.name}), METH_VARARGS|METH_KEYWORDS"
                )
            else:
                sf.write(f"func_{member.py_name.name}, METH_VARARGS")

            docstring = _optional_ptr(
                _has_member_docstring(bindings, member, spec.module.overloads),
...
def _append_qualifier_defines(module, bindings, qualifier_defines):
    ...
    for qualifier in module.qualifiers:
        ...
        if qualifier.type is QualifierType.TIME:
            ...
            if (
                qualifier.name not in bindings.disabled_features
                and qualifier.enabled_by_default
            ):
                qualifier_type_name = "FEATURE"

        if qualifier_type_name is not None:
            qualifier_defines.append(
                f"#define SIP_{qualifier_type_name}_{qualifier.name}"
            )
...
def _overload_auto_docstring(sf, spec, overload, is_method=True):
    ...
    signature = fmt_signature_as_type_hint(
        spec, overload.py_signature, need_self=need_self
    )
...
def _const_cast(spec, type, value):
    ...
    if type.is_const:
        cpp_type = fmt_argument_as_cpp_type(spec, type, plain=True, no_derefs=True)
...
def _callable_overloads(member, overloads):
    ...
    for overload in overloads:
        if (
            overload.common is member
            and overload.access_specifier is not AccessSpecifier.PRIVATE
            and overload.pyqt_method_specifier is not PyQtMethodSpecifier.SIGNAL
        ):
            yield overload
...
def _gto_name(wrapped_object):
    ...
    fq_cpp_name = (
        wrapped_object.fq_cpp_name
        if isinstance(wrapped_object, WrappedEnum)
        else wrapped_object.iface_file.fq_cpp_name
    )
...
def _unique_class_ctors(spec, klass):
    ...
    for ctor in klass.ctors:
        ...
        for do_ctor in klass.ctors:
            ...
            if do_ctor.cpp_signature is not None and same_signature(
                spec, do_ctor.cpp_signature, ctor.cpp_signature
            ):
                break
...
def _unique_class_virtual_overloads(spec, klass):
    ...
    for virtual_overload in klass.virtual_overloads:
        ...
        for do_virtual_overload in klass.virtual_overloads:
            ...
            if do_overload.cpp_name == overload.cpp_name and same_signature(
                spec, do_overload.cpp_signature, overload.cpp_signature
            ):
                break
...
def _module_supports_qt(spec):
    ...
    return (
        spec.pyqt_qobject is not None
        and spec.pyqt_qobject.iface_file.module is spec.module
    )
...
def _py_scope(scope):
    ...
    return (
        None if isinstance(scope, WrappedClass) and scope.is_hidden_namespace else scope
    )
...
def _release_gil(gil_action, bindings):
    ...
    return (
        bindings.release_gil
        if gil_action is GILAction.DEFAULT
        else gil_action is GILAction.RELEASE
    )
...
def _write_instances_table(sf, scope, instances, declaration_template):
    ...
    declaration = declaration_template.format(dict_type=dict_type, suffix=suffix)
    sf.write(f"\n\n{declaration} = {{\n")
...
class SourceFile:
    ...
    def open(self, source_name, project):
        ...
        self._write_header_comments(
            self._description, self._module, project.version_info
        )
    ...
    def write_code(self, code):
        ...
        for code_block in code_blocks:
            self.write(
                f'#line {code_block.line_nr} "{self._posix_path(code_block.sip_file)}"\n'
            )
...
class CompilationUnit(SourceFile):
    ...
    def __init__(
        self, source_name, description, module, project, buildable, sip_api_file=True
    ):
        """Initialise the object."""

        super().__init__(source_name, description, module, project, buildable.sources)
