        ...
        for klass in spec.classes:
            if klass.iface_file.module is module and not klass.external:
                ...
                for overload in klass.overloads:
                    if (
                        overload.access_specifier is not AccessSpecifier.PRIVATE
...
def _ctor(af, spec, module, ctor, scope):
    ...
    py_class = (
        module.py_name + "." + fmt_scoped_py_name(scope.scope, scope.py_name.name)
    )
...
def _get_py_results(spec, py_signature):
    ...
    if py_signature.result is not None:
        if (
            py_signature.result.type is not ArgumentType.VOID
            or len(py_signature.result.derefs) != 0
        ):
