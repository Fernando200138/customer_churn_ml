...
class TypeHintManager:
    ...
    def as_docstring(self, type_hint, out, context):
        ...
        if managed_type_hint.as_docstring is None:
            managed_type_hint.as_docstring = self._render(
                managed_type_hint, out, context=context
            )
    ...
    def as_rest_ref(self, type_hint, out, context, as_xml=False):
        ...
        if managed_type_hint.as_rest_ref is None:
            managed_type_hint.as_rest_ref = self._render(
                managed_type_hint, out, context=context, rest_ref=True, as_xml=as_xml
            )
    ...
    def as_type_hint(self, type_hint, out, context, defined):
        ...
        return self._render(
            managed_type_hint, out, pep484=True, context=context, defined=defined
        )
    ...
    def _parse(self, managed_type_hint, out):
        ...
        if managed_type_hint.parse_state is ParseState.REQUIRED:
            managed_type_hint.root = self._parse_node(out, managed_type_hint.type_hint)
    ...
    def _parse_node(self, out, text, start=0, end=None):
        ...
        if name_start != name_end:
            ...
            try:
                ...
                if name == "Union":
                    ...
                    for child in children:
                        if (
                            child.type is NodeType.TYPING
                            and child.definition == "Union"
                        ):
                ...
                pep484_definition = (
                    name if type_module is None else type_module + "." + name
                )
                ...
                node = TypeHintNode(
                    ...
                    children=children,
                    definition=name,
                    pep484_definition=pep484_definition,
                )
            ...
            if top_level and (children is None or len(children) == 0):
                raise UserException(f"type hint '{text}': must have non-empty brackets")
    ...
    def _render(
        ...
        if managed_type_hint.root is not None:
            ...
            s = self._maybe_any_object(managed_type_hint.type_hint, pep484, as_xml)

        return s

    def _render_node(self, node, out, pep484, rest_ref, defined, as_xml, context_stack):
        ...
        if node.type is NodeType.TYPING:
            ...
            if node.children is not None:
                s += self._render_children(
                    node, out, pep484, rest_ref, defined, as_xml, context_stack
                )
        ...
        if node.children is not None and defined is None:
            return node.definition + self._render_children(
                node, out, pep484, rest_ref, defined, as_xml, context_stack
            )
    ...
    def _render_children(
        self, node, out, pep484, rest_ref, defined, as_xml, context_stack
    ):
        ...
        for child in node.children:
            ...
            children.append(
                self._render_node(
                    child, fixed_out, pep484, rest_ref, defined, as_xml, context_stack
                )
            )
    ...
    def _lookup_enum(self, name, scopes):
        ...
        for enum in self._spec.enums:
            if (
                enum.fq_cpp_name is not None
                and enum.fq_cpp_name.base_name == name
                and enum.scope in scopes
            ):
                return enum
    ...
    def _lookup_class(self, name, scope):
        ...
        for klass in self._spec.classes:
            if (
                klass.scope is scope
                and not klass.external
                and klass.iface_file.fq_cpp_name.base_name == name
            ):
                return klass
        ...
        for part_i, part in enumerate(scoped_name):
            ...
            if scope_klass is None:
                ...
                if mapped_type is not None:
                    ...
                    if is_last_part:
                        return TypeHintNode(
                            NodeType.MAPPED_TYPE, definition=mapped_type
                        )
