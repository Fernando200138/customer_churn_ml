...
from .formatters import (
    fmt_argument_as_py_default_value,
    fmt_argument_as_rest_ref,
    fmt_class_as_rest_ref,
    fmt_scoped_py_name,
            ...
            if enum.py_name is None:
                for member in enum.members:
                    SubElement(
                        parent,
                        "Member",
                        name=fmt_scoped_py_name(enum.scope, member.py_name.name),
                ...
                enum_el = SubElement(
                    parent,
                    "Enum",
                    name=fmt_scoped_py_name(enum.scope, enum.py_name.name),
                    realname=_realname(enum.fq_cpp_name),
                )

                for member in enum.members:
                    name = (
                        fmt_scoped_py_name(enum.scope, enum.py_name.name)
                        + "."
                        + member.py_name.name
                    )
                    ...
                    SubElement(
                        ...
                        "EnumMember",
                        name=name,
                        realname=_realname(enum.fq_cpp_name, member.cpp_name),
                    )
    ...
    for variable in spec.variables:
        if variable.module is module and variable.scope is scope:
            ...
            SubElement(
                ...
                "Member",
                attrib,
                name=fmt_scoped_py_name(variable.scope, variable.py_name.name),
    ...
    for overload in overloads:
        if (
            ...
            if overload.pyqt_method_specifier is PyQtMethodSpecifier.SIGNAL:
                ...
                is_static = (
                    scope is None
                    or scope.iface_file.type is IfaceFileType.NAMESPACE
                    or overload.is_static
                )

                if (
                    scope is None
                    and member.py_slot is not None
                    and overload.py_signature.args[0].type is ArgumentType.CLASS
                ):
                    extends = overload.py_signature.args[0].definition
                    is_static = False

                _overload(parent, spec, scope, overload, extends, is_static)
...
def _overload(parent, spec, scope, overload, extends, is_static):
    ...
    if _has_cpp_signature(overload.cpp_signature):
        attrib["cppsig"] = _cpp_signature(
            spec, overload.cpp_signature, is_const=overload.is_const
        )
    ...
    if extends is not None:
        attrib["extends"] = fmt_scoped_py_name(extends.scope, extends.py_name.name)
    ...
    if (
        result.type_hints is not None
        and result.type_hints.hint_out is not None
        and result.type_hints.hint_out == ""
    ):
        no_result = True
    else:
    ...
    might_ignore = (
        is_number_slot(overload.common.py_slot) and len(overload.py_signature.args) == 2
    )
...
def _cpp_signature(spec, cpp_signature, is_const=False):
    ...
    args = fmt_signature_as_cpp_declaration(
        spec, cpp_signature, strip=STRIP_GLOBAL, make_public=True, as_xml=True
    )
...
def _argument(parent, spec, arg, kw_args, out=False, transfer_result=False):
...
def _typename(spec, arg, kw_args=KwArgs.NONE, out=False):
    ...
    if not out and arg.name is not None:
        if kw_args is KwArgs.ALL or (
            kw_args is KwArgs.OPTIONAL and arg.default_value is not None
        ):
            s += arg.name.name + ": "
    ...
    if not out and arg.name is not None and arg.default_value is not None:
        ...
        if rest_ref is None:
            rest_ref = fmt_argument_as_py_default_value(
                spec, arg, arg_rest_ref, as_xml=True
            )
