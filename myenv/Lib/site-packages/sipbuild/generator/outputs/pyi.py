...
def _class(pf, spec, klass, defined, indent=0):
    ...
    if not klass.is_hidden_namespace:
        ...
        if klass.superclasses:
            s += ", ".join(
                [fmt_class_as_type_hint(spec, sc, defined) for sc in klass.superclasses]
            )
            ...
            if (
                spec.target_abi[0] == 12
                and spec.sip_module
                and klass.supertype.name.startswith("sip.")
            ):
            ...
            simple = (
                "simple" if klass.iface_file.type is IfaceFileType.NAMESPACE else ""
            )
    ...
    for prop in klass.properties:
        ...
        if getter is not None:
            _property(pf, spec, prop, False, getter, klass.overloads, defined, indent)

            if prop.setter is not None:
                setter = find_method(klass, prop.setter)
                if setter is not None:
                    _property(
                        pf, spec, prop, True, setter, klass.overloads, defined, indent
                    )
...
def _mapped_type(pf, spec, mapped_type, defined):
    ...
    if not no_body:
        _separate(pf)
        pf.write(
            f"class {mapped_type.py_name.name}({_sip_module_name(spec)}wrapper):\n"
        )
        ...
        for member in mapped_type.members:
            ...
            _callable(
                pf, spec, member, member.overloads, defined, is_method=True, indent=1
            )
...
def _variables(pf, spec, defined, scope=None, indent=0):
    ...
    for variable in spec.variables:
        ...
        py_type = fmt_argument_as_type_hint(spec, variable.type, defined, arg_nr=None)
...
def _callable(pf, spec, member, overloads, defined, is_method=False, indent=0):
    ...
    for overload in nonreflected_overloads:
        _overload(
            pf, spec, overload, overloaded, first_overload, is_method, defined, indent
        )
...
def _overload(
    ...
    if is_eq_slot:
        ...
        signature = _python_signature(spec, py_signature, defined, need_self=need_self)
...
def _python_signature(spec, signature, defined, need_self=True):
    ...
    return fmt_signature_as_type_hint(
        spec, signature, need_self=need_self, defined=defined
    )
