...
def find_iface_file(
    spec, mod, fq_cpp_name, iface_file_type, error_logger, cpp_type=None, scope=None
):
    ...
    for iff in spec.iface_files:
        ...
        if iff.type is not iface_file_type:
            if (
                iface_file_type is not IfaceFileType.EXCEPTION
                or iff.type is not IfaceFileType.CLASS
            ):
                error_logger(
        ...
        if (
            iface_file_type is IfaceFileType.CLASS
            and iff.module is not None
            and iff.module is not mod
        ):
        ...
        if iface_file_type == IfaceFileType.MAPPED_TYPE and iff.module is not mod:
            for mtd in spec.mapped_types:
                ...
                if (
                    cpp_type.type is not ArgumentType.TEMPLATE
                    or mtd.type.type is not ArgumentType.TEMPLATE
                    or same_base_type(cpp_type, mtd.type)
                ):
                    error_logger(
...
def same_argument_type(spec, arg1, arg2, strict=True):
    ...
    if spec.target_abi >= (13, 0):
        ...
        if (
            arg1.type in _PY_INT
            and arg2.type is ArgumentType.ENUM
            and arg2.definition.fq_cpp_name is None
        ):
            return True

        if (
            arg1.type is ArgumentType.ENUM
            and arg1.definition.fq_cpp_name is None
            and arg2.type in _PY_INT
        ):
            return True
    else:
        ...
        if (
            arg1.type in _PY_INT
            and arg2.type is ArgumentType.ENUM
            and not arg2.is_constrained
        ):
            return True

        if (
            arg1.type is ArgumentType.ENUM
            and not arg1.is_constrained
            and arg2.type in _PY_INT
        ):
            return True
