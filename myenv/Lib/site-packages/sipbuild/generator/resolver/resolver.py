...
def resolve(spec, modules):
    ...
    for klass in spec.classes:
        if klass.iface_file.module is None:
            error_log.log(
                "class '{0}' has not been defined".format(klass.iface_file.fq_cpp_name)
            )
    ...
    for klass in spec.classes:
        if (
            klass.no_default_ctors
            or klass.is_opaque
            or klass.iface_file.type is IfaceFileType.NAMESPACE
        ):
            continue
    ...
    for klass in spec.classes:
        if klass.iface_file.type is IfaceFileType.CLASS:
            if (
                klass.needs_shadow
                and not klass.is_incomplete
                and klass.dtor is not AccessSpecifier.PRIVATE
                and klass.can_create
            ):
            ...
            for overload in klass.overloads:
                _iface_files_are_used_by_overload(spec, klass.iface_file.used, overload)
    ...
    for exception in spec.exceptions:
        ...
        # module.
        if spec.target_abi >= (13, 1) or (
            spec.target_abi >= (12, 9) and spec.target_abi < (13, 0)
        ):
        ...
        if (
            exception.builtin_base_exception is None
            and exception.defined_base_exception is None
        ):
            continue
    ...
    for klass in spec.classes:
        if klass.iface_file.module is mod:
            ...
            _resolve_scope_overloads(
                spec, klass.overloads, error_log, final_checks, scope=klass
            )
...
def _add_complementary_slot(spec, klass, member, compl, compl_name):
    ...
    for overload in klass.overloads:
        if (
            ...
            or overload.is_complementary
            or overload.method_code is not None
        ):
            continue

        # Try and find an existing complementary slot.
        for overload2 in klass.overloads:
            if overload2.common.py_slot is compl and same_signature(
                spec, overload.py_signature, overload2.py_signature
            ):
                break
        else:
...
def _check_helpers(spec, klass):
    ...
    for ctor in klass.ctors:
        if (
            ctor.cpp_signature is None
            or ctor.access_specifier is not AccessSpecifier.PUBLIC
        ):
            continue

        if (
            len(ctor.cpp_signature.args) == 0
            or ctor.cpp_signature.args[0].default_value is not None
        ):
            ...
            if (
                arg.definition is klass
                and arg.is_reference
                and arg.is_const
...
def _set_all_imports(mod, error_log, seen=None):
    ...
    if seen is None:
        ...
        error_log.log("module '{0}' is imported recursively".format(mod.fq_py_name))
...
def _move_class_casts(spec, klass, error_log):
    ...
    for cast in klass.casts:
        ...
        arg = Argument(
            ArgumentType.CLASS,
            definition=klass,
            derefs=list(cast.derefs),
            is_reference=cast.is_reference,
            is_const=cast.is_const,
            is_in=True,
        ...
        for dst_ctor in dst_klass.ctors:
            if same_signature(
                spec, dst_ctor.py_signature, ctor.py_signature, strict=False
            ):
                error_log.log(
                    " operator '{0}::{0}({1})' already defined".format(
                        dst_klass.iface_file.fq_cpp_name, klass.iface_file.fq_cpp_name
                    )
                )
    ...
    for overload in list(spec.module.overloads):
        ...
        if arg0.type is ArgumentType.CLASS:
            ...
            if arg0.type is ArgumentType.NONE:
                ...
                _log_overload_error(
                    error_log, "the argument must be a class or enum", overload
                )
            continue
            ...
            if arg0.type is ArgumentType.NONE or arg1.type is ArgumentType.NONE:
                ...
                _log_overload_error(
                    error_log, "one of the arguments must be a class or enum", overload
                )
            continue
        ...
        if is_rich_compare_slot(global_slot.py_slot):
            if is_second:
                _log_overload_error(
                    error_log, "argument 1 must be a class or enum", overload
                )
                continue

            if arg_module is not global_slot.module and arg0.type is ArgumentType.ENUM:
                _log_overload_error(error_log, "argument 1 must be a class", overload)
                continue
        ...
        for arg_member in arg_members:
            ...
            if (
                spec.target_abi < (13, 0)
                and arg0.type is ArgumentType.ENUM
                and arg_member.py_slot is PySlot.EQ
                and not is_second
            ):
        ...
        # operators.
        if is_inplace_number_slot(arg_member.py_slot) or is_rich_compare_slot(
            arg_member.py_slot
        ):
...
def _set_mro(spec, klass, error_log, seen=None):
            ...
            if superklass in seen:
                error_log.log(
                    "recursive class hierarchy detected: '{0}' and '{1}'".format(
                        klass.iface_file.fq_cpp_name, superklass.iface_file.fq_cpp_name
                    )
                )
                continue
    ...
    if klass.subclass_base is not None:
        append_iface_file(klass.iface_file.module.used, klass.subclass_base.iface_file)
    ...
    if (
        klass.is_incomplete
        or klass.dtor is AccessSpecifier.PRIVATE
        or not klass.can_create
    ):
        ...
        for overload in klass.overloads:
            if (
                ...
                and overload.access_specifier is AccessSpecifier.PRIVATE
            ):
                klass.has_shadow = False
...
def _resolve_typedefs(spec, mod, error_log):
    ...
    for typedef in spec.typedefs:
        if typedef.module is mod:
            _resolve_type(spec, typedef.module, typedef.scope, typedef.type, error_log)
    ...
    for ctor in klass.ctors:
        ...
        if ctor.method_code is None:
            for previous_ctor in klass.ctors:
                ...
                sig_state = _same_python_signature(
                    spec, previous_ctor.py_signature, ctor.py_signature
                )
...
def _add_default_copy_ctor(klass):
        ...
        for ctor in mro_klass.ctors:
            ...
            if len(arg.derefs) == 0 and arg.is_reference:
                if (
                    ...
                    and arg.definition.iface_file is mro_klass.iface_file
                ):
                    break
        else:
            continue
    ...
    ctor = Constructor(
        AccessSpecifier.PUBLIC, py_signature=signature, cpp_signature=signature
    )
...
def _resolve_scope_overloads(spec, overloads, error_log, final_checks, scope=None):
    """Resolve the data types for a scope's overloads."""

    for overload in overloads:
        _resolve_func_types(
            spec, overload.common.module, scope, overload, error_log, final_checks
        )
        ...
        if overload.method_code is None and spec.is_strict:
            for previous_overload in overloads:
                ...
                sig_state = _same_python_signature(
                    spec, previous_overload.py_signature, overload.py_signature
                )
...
def _resolve_enums(spec, error_log):
    ...
    for enum in spec.enums:
        ...
        if base_type.type not in _ENUM_BASE_TYPES or len(base_type.derefs) != 0:
            error_log.log(
                f"unsupported enum base type", source_location=base_type.source_location
            )
            ...
            for visible_member in klass.visible_members:
                ...
                for overload in mro_klass.overloads:
                    if overload.common is member:
                        ...
                        if klass.iface_file.module is spec.module and (
                            klass is mro_klass
                            or (
                                overload.access_specifier is AccessSpecifier.PROTECTED
                                and klass.has_shadow
                            )
                        ):
                            need_types = True
                            member.py_name.used = True
                        ...
                        _iface_files_are_used_by_overload(
                            spec, klass.iface_file.used, overload, need_types=need_types
                        )
...
def _get_virtuals(spec, klass, error_log):
    ...
    for superklass in klass.superclasses:
        for virtual_overload in superklass.virtual_overloads:
                ...
                if _same_cpp_overload(spec, virtual_overload.overload, overload):
                    ...
                    if overload.virtual_catcher_code is None:
                        overload.virtual_catcher_code = (
                            virtual_overload.overload.virtual_catcher_code
                        )
                    ...
                    if overload.virtual_error_handler is None:
                        overload.virtual_error_handler = (
                            virtual_overload.overload.virtual_error_handler
                        )
            ...
            if implicit:
                _add_virtual_overload(spec, virtual_overload.overload, klass, error_log)

    # Handle any new virtuals.
    for overload in klass.overloads:
        if (
            overload.is_virtual
            and not overload.is_virtual_reimplementation
            and not overload.is_final
        ):
            _add_virtual_overload(spec, overload, klass, error_log)
...
def _add_virtual_overload(spec, overload, klass, error_log):
    ...
    # have a handler.
    if klass.iface_file.module is spec.module:
        virtual_handler = _get_virtual_handler(spec, overload, klass, error_log)
        ...
        # virtual handler.
        _iface_files_are_used_by_overload(
            spec, spec.module.used, overload, need_types=True
        )
    else:
...
def _get_virtual_error_handler(spec, overload, klass, error_log):
    ...
    for handler in spec.virtual_error_handlers:
        ...
        error_log.log("unknown virtual error handler '{0}'".format(handler_name))
        return None
...
def _check_virtual_handler(spec, overload, virtual_handler):
    ...
    if overload.virtual_error_handler is not None:
        if (
            virtual_handler.virtual_error_handler is None
            or overload.virtual_error_handler
            != virtual_handler.virtual_error_handler.name
        ):
            return False

    if (
        overload.factory or overload.transfer is Transfer.TRANSFER_BACK
    ) and not virtual_handler.transfer_result:
        return False

    if overload.abort_on_exception is not virtual_handler.abort_on_exception:
        return False

    if not same_argument_type(
        spec, overload.py_signature.result, virtual_handler.py_signature.result
    ):
        return False

    if (
        overload.py_signature.result.allow_none
        is not virtual_handler.py_signature.result.allow_none
    ):
        return False

    if (
        overload.py_signature.result.disallow_none
        is not virtual_handler.py_signature.result.disallow_none
    ):
        return False
    ...
    for arg1, arg2 in zip(
        overload.py_signature.args, virtual_handler.py_signature.args
    ):
    ...
    if (
        overload.py_signature is overload.cpp_signature
        and virtual_handler.py_signature is virtual_handler.cpp_signature
    ):
        return True

    if not same_argument_type(
        spec, overload.cpp_signature.result, virtual_handler.cpp_signature.result
    ):
        return False

    return same_signature(spec, overload.cpp_signature, virtual_handler.cpp_signature)
...
def _resolve_mapped_type_types(spec, mapped_type, error_log):
    ...
    _iface_files_are_used_by_signature(mapped_type.iface_file.used, template_types)
...
def _resolve_ctor_types(spec, scope, ctor, error_log):
    ...
    if ctor.cpp_signature is not None and ctor.cpp_signature is not ctor.py_signature:
        for arg in ctor.cpp_signature.args:
            _resolve_type(
                spec, scope.iface_file.module, scope, arg, error_log, allow_defined=True
            )
...
def _resolve_func_types(spec, mod, scope, overload, error_log, final_checks):
    ...
    if overload.cpp_signature is not overload.py_signature:
        ...
        if (
            (result.type is not ArgumentType.VOID or len(result.derefs) != 0)
            and overload.is_virtual
            and not _supported_type(scope, overload, result, error_log)
            and overload.virtual_catcher_code is None
        ):
            _log_overload_error(
                error_log,
    ...
    # Handle the Python signature.
    _resolve_py_signature_types(spec, mod, scope, overload, error_log, final_checks)
    ...
    if is_ssize_return_slot(overload.common.py_slot):
        ...
        if (
            result.type not in required_types
            or len(result.derefs) != 0
            or result.is_reference
            or result.is_const
        ):
            _log_overload_error(
                error_log, "must return a Py_ssize_t", overload, scope=scope
            )

    # These slots must return int.
    if is_int_return_slot(overload.common.py_slot):
        if (
            result.type is not ArgumentType.INT
            or len(result.derefs) != 0
            or result.is_reference
            or result.is_const
        ):
            _log_overload_error(error_log, "must return an int", overload, scope=scope)

    # These slots must return void.
    if is_void_return_slot(overload.common.py_slot):
        if (
            result.type is not ArgumentType.VOID
            or len(result.derefs) != 0
            or result.is_reference
            or result.is_const
        ):
            _log_overload_error(error_log, "must return void", overload, scope=scope)
    ...
    if is_hash_return_slot(overload.common.py_slot):
        ...
        if (
            result.type is not required_type
            or len(result.derefs) != 0
            or result.is_reference
            or result.is_const
        ):
            _log_overload_error(
                error_log,
                "must return a {0}".format(required_type_name),
                overload,
                scope=scope,
            )


def _resolve_py_signature_types(spec, mod, scope, overload, error_log, final_checks):
    ...
    if result.type is not ArgumentType.VOID or len(result.derefs) != 0:
        if overload.pyqt_method_specifier is PyQtMethodSpecifier.SIGNAL:
            _log_overload_error(
                error_log, "is a signal and must return void", overload, scope=scope
            )
        ...
        if result.type is ArgumentType.NONE:
            ...
            if (
                overload.cpp_signature is overload.py_signature
                or overload.method_code is None
            ):
                _log_overload_error(
                    error_log,
    ...
    for arg_nr, arg in enumerate(overload.py_signature.args):
        ...
        if (
            arg.type in (ArgumentType.CLASS, ArgumentType.MAPPED)
            and arg.array is ArrayArgument.ARRAY
        ):
            # This is delayed because the class must be complete.  Be careful
            # not to pass any non-local changing variables.
            local_arg_nr = arg_nr
            final_checks.append(
                lambda: _check_array_support(overload, local_arg_nr, scope, error_log)
            )
...
def _check_array_support(overload, arg_nr, scope, error_log):
    ...
    if arg.type is ArgumentType.CLASS and not arg.definition.needs_array_helper:
        _log_overload_error(
            error_log,
            "argument {0} is a class that does not support /Array/".format(arg_nr + 1),
            overload,
            scope=scope,
        )
...
def _resolve_variable_type(spec, variable, error_log):
    ...
    _resolve_type(spec, variable.module, variable.scope, variable_type, error_log)
    ...
    if bad_type and (
        variable.get_code is None
        or (not variable.no_setter and variable.set_code is None)
    ):
    ...
    if (
        variable_type.type is not ArgumentType.CLASS
        and variable.access_code is not None
    ):
        error_log.log(
...
def _ensure_input(klass, overload, arg, error_log):
    ...
    if arg.is_out:
        _log_overload_error(
            error_log, "has an invalid argument type for /Out/", overload, scope=klass
        )
...
def _same_cpp_overload(spec, overload1, overload2):
    ...
    return same_signature(spec, overload1.cpp_signature, overload2.cpp_signature)
    ...
    while type.type is ArgumentType.DEFINED:
        ...
        if type.type is ArgumentType.NONE:
            if allow_defined:
                ...
                error_log.log(
                    "'{0}' is undefined".format(scoped_name),
    ...
    if type.type in (ArgumentType.STRUCT, ArgumentType.UNION, ArgumentType.TEMPLATE):
        ...
        if type.type is ArgumentType.TEMPLATE:
            for mapped_type_template in spec.mapped_type_templates:
                if (
                    mapped_type_template.mapped_type.type.definition.cpp_name
                    == type.definition.cpp_name
                    and same_template_signature(
                        mapped_type_template.mapped_type.type.definition.types,
                        type.definition.types,
                        deep=True,
                    )
                ):
                    _instantiate_mapped_type_template(
                        spec, mod, mapped_type_template, type, error_log
                    )
                    break
...
def _set_needed_exceptions(spec, mod, throw_args):
    ...
    if (
        ...
        and throw_args.arguments is not None
    ):
...
def _resolve_instantiated_class_template(spec, type):
    ...
    for klass in spec.classes:
        if (
            klass.template is not None
            and klass.template.cpp_name == template.cpp_name
            and same_signature(spec, klass.template.types, template_signature)
        ):
...
def _instantiate_mapped_type_template(spec, mod, mapped_type_template, type, error_log):
    ...
    iface_file = find_iface_file(
        spec,
        mod,
        encoded_template_name(type.definition),
        IfaceFileType.MAPPED_TYPE,
        error_log.log,
        cpp_type=type,
    )
    ...
    if proto_mapped_type.type_hints is not None:
        mapped_type.type_hints = instantiate_type_hints(
            spec, proto_mapped_type.type_hints, expansions
        )

    used = mapped_type.iface_file.used

    mapped_type.iface_file.type_header_code = template_code_blocks(
        spec, used, proto_mapped_type.iface_file.type_header_code, expansions
    )

    if proto_mapped_type.convert_from_type_code is not None:
        mapped_type.convert_from_type_code = template_code(
            spec, used, proto_mapped_type.convert_from_type_code, expansions
        )

    if proto_mapped_type.convert_to_type_code is not None:
        mapped_type.convert_to_type_code = template_code(
            spec, used, proto_mapped_type.convert_to_type_code, expansions
        )

    if proto_mapped_type.release_code is not None:
        mapped_type.release_code = template_code(
            spec, used, proto_mapped_type.release_code, expansions
        )
    ...
    if mapped_type.type.type is ArgumentType.TEMPLATE:
        ...
        for arg_nr, arg in enumerate(type.definition.types.args):
            ...
            if dst_types is None:
                ...
                mapped_type.type.definition.types = copy(
                    mapped_type.type.definition.types
                )
                mapped_type.type.definition.types.args = [
                    copy(arg) for arg in mapped_type.type.definition.types.args
                ]
...
def _search_classes(spec, mod, scoped_name, type):
    ...
    for klass in spec.classes:
        ...
        if klass.iface_file.fq_cpp_name == scoped_name:
            type.type = ArgumentType.CLASS
            type.definition = klass
            type.type_hints = _merged_type_hints(type.type_hints, klass.type_hints)
...
def _iface_files_are_used_by_overload(spec, used, overload, need_types=False):
    ...
    _iface_files_are_used_by_signature(
        used, overload.py_signature, need_types=need_types
    )

    if overload.cpp_signature is not overload.py_signature:
        _iface_files_are_used_by_signature(
            used, overload.cpp_signature, need_types=need_types
        )
...
def _create_sorted_numbered_types(spec, mod, error_log):
    ...
    for klass in spec.classes:
        ...
        if mod is spec.module or klass.iface_file.needed:
            if not klass.is_hidden_namespace:
                mod.needed_types.append(
                    Argument(
                        ArgumentType.CLASS,
                        definition=klass,
                        name=klass.iface_file.cpp_name,
                    )
                )
    ...
    for enum in spec.enums:
        ...
        if mod is spec.module or enum.needed:
            mod.needed_types.append(
                Argument(
                    ArgumentType.ENUM, definition=enum, name=enum.cached_fq_cpp_name
                )
            )
        ...
        if needed_type.type is ArgumentType.CLASS:
            ...
            if needed_type.name.name == "QObject":
                if spec.pyqt_qobject is not None:
                    error_log.log("class 'QObject' has been defined more than once")
...
def _log_overload_error(error_log, text, overload, scope=None):
    ...
    error_log.log(f"'{fq_cpp_name}' {text}", source_location=overload.source_location)
