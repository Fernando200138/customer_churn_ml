...
def get_bindings_configuration(spec, sip_file, sip_include_dirs):
    ...
    for sip_dir in sip_include_dirs:
        toml_file = os.path.join(sip_dir, bindings_name, bindings_name + ".toml")
    ...
    if cfg_abi_version is None or not isinstance(cfg_abi_version, str):
        raise UserFileException(
            toml_file, "'sip-abi-version' must be specified as a string"
        )
    ...
    return (
        _get_string_list(toml_file, cfg, "module-tags"),
...
def _get_string_list(toml_file, cfg, name):
    ...
    for option in option_list:
        if not isinstance(option, str):
            raise UserFileException(
                toml_file, "elements of '{0}' must be strings".format(name)
            )
