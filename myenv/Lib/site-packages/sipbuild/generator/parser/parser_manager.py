...
class ParserManager:
    ...
    def complete_class(self, p, symbol, annotations, has_body):
        ...
        if has_body:
            if klass.scope is not None:
                if (
                    klass.iface_file.fq_cpp_name.scope
                    != klass.scope.iface_file.fq_cpp_name
                ):
                    self.parser_error(
                        p,
                        symbol,
            ...
            self.parser_error(
                p, symbol, "the class/struct has super-classes but no definition"
            )
        else:
            klass.is_opaque = True

        # Get the Python name and see if it is different to the C++ name.
        py_name = self.get_py_name(klass.iface_file.fq_cpp_name.base_name, annotations)
        ...
        pyqt_flags_enums = self._get_plugin_annotation(
            p, symbol, annotations, "PyQtFlagsEnums", "PyQt5"
        )
        if pyqt_flags_enums is not None:
            klass.pyqt_flags_enums = pyqt_flags_enums
            klass.pyqt_flags = 1

        pyqt_flags = self._get_plugin_annotation(
            p, symbol, annotations, "PyQtFlags", "PyQt5"
        )
        ...
        if klass.is_opaque:
            ...
            if klass.default_ctor is None:
                ...
                for ctor in klass.ctors:
                    ...
                    if (
                        len(ctor.py_signature.args) == 0
                        or ctor.py_signature.args[0].default_value is not None
                    ):
                        klass.default_ctor = ctor
                        break
            ...
            if klass.convert_to_type_code is not None and annotations.get(
                "AllowNone", False
            ):
    ...
    def define_class(
        self, p, symbol, class_key, scoped_name, annotations, superclasses=None
    ):
        ...
        self.push_scope(
            ...
            (
                ...
                if class_key is ClassKey.CLASS
    ...
    def find_class(self, p, symbol, iface_file_type, fq_cpp_name, tmpl_arg=False):
    ...
    def find_exception(self, p, symbol, fq_cpp_name, raise_code=None):
        ...
        iface_file = self.find_iface_file(
            p, symbol, fq_cpp_name, IfaceFileType.EXCEPTION
        )
        ...
        # Create a new one.
        w_exception = WrappedException(
            iface_file, raise_code, class_exception=class_exception
        )
    ...
    def new_class(
        ...
        if scope is not None:
            if (
                self.scope_access_specifier is AccessSpecifier.PROTECTED
                and not self._protected_is_public
            ):
        ...
        if (
            iface_file_type is not IfaceFileType.NAMESPACE
            and klass.iface_file.module is not None
        ):
            self.parser_error(
                p, symbol, "the class/struct/union has already been defined"
            )
    ...
    def _find_class_with_iface_file(self, iface_file, tmpl_arg=False):
        ...
        # Create a new one.
        klass = WrappedClass(
            iface_file, cached_name(self.spec, iface_file.fq_cpp_name.base_name), None
        )
    ...
    def add_ctor(
        ...
        *,
        ...
        if p[symbol] != scope.iface_file.fq_cpp_name.base_name:
            self.parser_error(
                p, symbol, "constructor doesn't have the same name as its class"
            )
        ...
        # signature.
        py_signature = Signature(args=arg_list, result=Argument(ArgumentType.VOID))
        ...
        if access_specifier is not AccessSpecifier.PRIVATE:
            ctor.kw_args = self._get_kw_args(p, symbol, annotations, py_signature)
        ...
        if method_code is None and annotations.get("NoRaisesPyException") is None:
            if self.module_state.all_raise_py_exception or annotations.get(
                "RaisesPyException", False
            ):
    ...
    def add_dtor(
        ...
        *,
        ...
        if name != scope.iface_file.fq_cpp_name.base_name:
            self.parser_error(
                p, symbol, "destructor doesn't have the same name as its class"
            )
        ...
        if bool(self.c_bindings) and method_code is None:
            self.parser_error(
                p, symbol, "destructors in C modules must specify %MethodCode"
            )
        ...
        if self.parsing_virtual:
            if self.scope.class_key is ClassKey.UNION:
                self.parser_error(p, symbol, "a union cannot have a virtual destructor")
            ...
            self.parser_error(p, symbol, "an abstract destructor must be virtual")
    ...
    def add_enum(
        self, p, symbol, cpp_name, is_scoped, enum_base_type, annotations, members
    ):
        ...
        if base_type_s is not None:
            ...
            if self.spec.target_abi is not None and self.spec.target_abi[0] < 13:
                self.parser_error(
                    p, symbol, "/BaseType/ is only supported for ABI v13.0 and later"
                )
            ...
            if base_type_s == "Enum":
                ...
                self.parser_error(
                    p, 1, "'{0}' is an invalid value of /BaseType/".format(base_type_s)
                )
        ...
        if cpp_name is None:
            ...
            py_name = cached_name(self.spec, self.get_py_name(cpp_name, annotations))
        ...
        w_enum = WrappedEnum(
            ...
            cached_fq_cpp_name=cached_fq_cpp_name,
            is_scoped=is_scoped,
        ...
        for m_cpp_name, m_py_name, m_no_type_hint in members:
            ...
            w_enum.members.append(
                WrappedEnumMember(
                    m_cpp_name, m_py_name, w_enum, no_type_hint=m_no_type_hint
                )
            )
    ...
    def add_function(
        ...
        *,
        ...
        member = self._find_member(
            p, symbol, py_name, arg_list, annotations, method_code
        )

        # Create the overload.
        overload = Overload(
            self.scope_access_specifier, member, cpp_name, cpp_signature, py_signature
        )
        ...
        overload.abort_on_exception = annotations.get("AbortOnException", False)
        ...
        if overload.access_specifier is not AccessSpecifier.PRIVATE:
            if member.py_slot is None or member.py_slot is PySlot.CALL:
                overload.kw_args = self._get_kw_args(
                    p, symbol, annotations, py_signature, need_name=member.has_protected
                )
                ...
                if (
                    overload.pyqt_method_specifier is not PyQtMethodSpecifier.SIGNAL
                    and overload.access_specifier is AccessSpecifier.PROTECTED
                    and not self.in_main_module
                ):
                    for kwod in self.scope.overloads:
                        ...
                        for arg in kwod.py_signature.args:
                            if (
                                kwod.kw_args is KwArgs.OPTIONAL
                                and arg.default_value is None
                            ):
                                continue
        ...
        if annotations.get("Numeric", False):
            if member.is_sequence:
                self.parser_error(
                    p, symbol, "an overload has already specified /Sequence/"
                )
            else:
        ...
        if annotations.get("Sequence", False):
            if member.is_numeric:
                self.parser_error(
                    p, symbol, "an overload has already specified /Numeric/"
                )
            else:
                member.is_sequence = True

        self.apply_common_argument_annotations(
            p, symbol, overload.py_signature.result, annotations
        )
    ...
    def add_mapped_type(self, p, symbol, cpp_type, annotations):
        ...
        iface_file = self.find_iface_file(
            p, symbol, fq_cpp_name, IfaceFileType.MAPPED_TYPE, cpp_type=cpp_type
        )
        ...
        for mtd in self.spec.mapped_types:
            if mtd.iface_file is iface_file:
                ...
                # arguments.
                if cpp_type.type is not ArgumentType.TEMPLATE or same_base_type(
                    cpp_type, mtd.type
                ):
                    self.parser_error(
                        p,
        ...
        mapped_type.cpp_name = cached_name(self.spec, argument_as_str(cpp_type))

        if cpp_name is not None:
            mapped_type.py_name = cached_name(
                self.spec, annotations.get("PyName", cpp_name)
            )
    ...
    def add_qualifier(self, p, symbol, name, type, order=0, timeline=0):
        ...
        if qualifier is not None:
            ...
            if (
                qualifier.type is not QualifierType.TIME
                or type is not QualifierType.TIME
                or (qualifier.module is module and qualifier.timeline == timeline)
            ):
                self.parser_error(
                    p,
                    symbol,
                    "'{0}' has already been defined as a qualifier".format(name),
                )

                return

        qualifier = Qualifier(module, name, type, order=order, timeline=timeline)
            ...
            for td in self.spec.typedefs:
                if td.fq_cpp_name == typedef.fq_cpp_name:
                    self.parser_error(
                        p,
                        symbol,
                        "'{0}' has already been defined".format(typedef.fq_cpp_name),
                    )
    ...
    def annotate_mapped_type(self, p, symbol, mapped_type, annotations):
        ...
        pyqt_flags = self._get_plugin_annotation(
            p, symbol, annotations, "PyQtFlags", "PyQt6"
        )
        if pyqt_flags is not None:
    ...
    def apply_common_argument_annotations(self, p, symbol, arg, annotations):
        ...
        try:
            ...
            if key is None:
                ...
                self.parser_error(p, symbol, "a /KeepReference/ key cannot be negative")
    ...
    def apply_type_annotations(self, p, symbol, type, annotations):
        ...
        can_be_encoded = (
            type.type is ArgumentType.STRING
            and type.array is ArrayArgument.NONE
    ...
    def check_annotations(self, p, symbol, context, annotations):
        ...
        for name in p[symbol]:
            if name not in annotations:
                self.parser_error(
                    p, symbol, "{0} is not a valid {1} annotation".format(name, context)
                )

    def check_attributes(self, p, symbol, py_name, is_function=False, ignore=None):
        ...
        def clash(thing):
            self.parser_error(
                p,
                symbol,
                "there is already {0} in scope called '{1}'".format(thing, py_name),
            )
    ...
    def cpp_only(self, p, symbol, feature):
        ...
        if bool(self.c_bindings):
            self.parser_error(p, symbol, feature + " are not allowed in a C module")
    ...
    def convert_docstring_format(self, p, symbol):
        ...
        self.parser_error(
            p, symbol, "'{0}' is not a valid value for a docstring format".format(value)
        )
    ...
    def convert_docstring_signature(self, p, symbol):
        ...
        self.parser_error(
            p,
            symbol,
            "'{0}' is not a valid value for a docstring signature".format(value),
        )
    ...
    def convert_encoding(self, p, symbol, value=None):
        ...
        self.parser_error(p, symbol, "'{0}' is not a valid encoding".format(value))
    ...
    def convert_kw_args(self, p, symbol, value=None):
        ...
        self.parser_error(
            p, symbol, "'{0}' is not a valid value for keyword arguments".format(value)
        )
    ...
    def evaluate_feature_or_platform(self, p, symbol, name=None, inverted=False):
        ...
        if qual.type is QualifierType.FEATURE:
            ...
            self.parser_error(
                p,
                symbol,
                "'{0}' is a %Timeline qualifier which can only be used in a range".format(
                    name
                ),
            )
            return False
    ...
    def evaluate_timeline(self, p, symbol_lower, symbol_upper):
        ...
        if lower_qual is None and upper_qual is None:
            self.parser_error(
                p, symbol_lower, "the lower and upper bounds cannot both be omitted"
            )
            return False

        if lower_qual is not None and upper_qual is not None:
            if (
                lower_qual.module is not upper_qual.module
                or lower_qual.timeline != upper_qual.timeline
            ):
                self.parser_error(
                    p,
                    symbol_lower,
                    "'{0}' and '{1}' are defined in different %Timelines".format(
            ...
            if lower_qual is upper_qual:
                self.parser_error(
                    p, symbol_lower, "the lower and upper bounds must be different"
                )
                return False
            ...
            if lower_qual.order > upper_qual.order:
                self.parser_error(
                    p,
                    symbol_lower,
                    "'{0}' is later in the %Timeline than '{1}'".format(
                        lower_name, upper_name
                    ),
                )
                return False
        ...
        for qual in module.qualifiers:
            if (
                qual.type is QualifierType.TIME
                and qual.timeline == timeline
                and qual.name in self.tags
            ):
    ...
    def find_qualifier(self, p, symbol, name, required=True):
        ...
        if name.startswith("SIP_"):
            ...
            if order >= 0:
                ...
                qualifier = Qualifier(module, name, QualifierType.TIME, order=order)
                module.qualifiers.append(qualifier)

                return qualifier

        if required:
            self.parser_error(p, symbol, "'{0}' is not a known qualifier".format(name))
    ...
    def get_transfer(self, p, symbol, annotations):
        ...
        if transfer is None:
            self.parser_error(
                p, symbol, "Only one transfer annotation may be specified"
            )
            transfer = Transfer.NONE
    ...
    def get_type_hints(self, p, symbol, annotations):
        ...
        if th_in is None:
            ...
            self.parser_error(
                p, symbol, "'TypeHint' and 'TypeHintIn' cannot both be specified"
            )
        ...
        if th_out is None:
            ...
            self.parser_error(
                p, symbol, "'TypeHint' and 'TypeHintOut' cannot both be specified"
            )
        ...
        if th_in is not None or th_out is not None or th_value is not None:
            ...
            if annotations.get("NoTypeHint") is not None:
                self.parser_error(
                    p, symbol, "'NoTypeHint' cannot be specified with a type hint"
                )
    ...
    def instantiate_class_template(
        self, p, symbol, fq_cpp_name, template, py_name, no_type_name, docstring
    ):
        ...
        for tmpl_names, proto_class in self.class_templates:
            if proto_class.iface_file.fq_cpp_name.matches(
                template.cpp_name, scope=self.scope
    ...
    def parse(self, sip_file):
        ...
        try:
            self._parser.parse(
                self._read(sip_file, raw_sip_file), lexer=self._lexer, tracking=True
            )
    ...
    def parser_error(self, p, symbol, text):
        ...
        self.parser_error_at_location(self.get_source_location(p, symbol), text)
    ...
    def pop_module_state(self):
        ...
        if self._pending_module_state.default_encoding is None:
            self._pending_module_state.default_encoding = (
                self.module_state.default_encoding
            )

        # Inherit any call_super_init.
        if self._pending_module_state.call_super_init is None:
            self._pending_module_state.call_super_init = (
                self.module_state.call_super_init
            )
    ...
    def push_file(self, p, symbol, sip_file=None, new_module=False, optional=False):
        ...
        if os.path.isfile(sip_file):
            ...
            if found is None:
                if not optional:
                    self.parser_error(
                        p, symbol, "'{0}' could not be found".format(raw_sip_file)
                    )
        ...
        for detail in self._file_stack:
            if detail[0] == sip_file:
                self.parser_error(
                    p, symbol, "'{0}' is being read recursively".format(sip_file)
                )
                return
    ...
    def scope_access_specifier(self):
        ...
        return (
            ...
            else self._scope_stack[-1].access_specifier
        )
    ...
    def scope_pyqt_method_specifier(self):
        ...
        return (
            None
            if len(self._scope_stack) == 0
            else self._scope_stack[-1].pyqt_method_specifier
        )
    ...
    def validate_annotation(self, p, symbol, value):
        ...
        try:
            value = validate_annotation_value(self, p, symbol, p[symbol], value)
    ...
    def validate_function(self, p, symbol, overload):
        ...
        if overload.is_virtual:
            if (
                overload.virtual_error_handler is not None
                and overload.no_virtual_error_handler
            ):
                error(
                    "/VirtualErrorHandler/ and /NoVirtualErrorHandler/ cannot both be specified"
                )
        else:
            if overload.abort_on_exception:
                error(
                    "/AbortOnException/ cannot be specified for non-virtual member functions"
                )

            if overload.new_thread:
                error(
                    "/NewThread/ cannot be specified for non-virtual member functions"
                )

            if overload.virtual_call_code is not None:
                error(
                    "%VirtualCallCode cannot be specified for non-virtual member functions"
                )

            if overload.virtual_catcher_code is not None:
                error(
                    "%VirtualCatcherCode cannot be specified for non-virtual member functions"
                )

            if overload.virtual_error_handler is not None:
                error(
                    "/VirtualErrorHandler/ cannot be specified for non-virtual member functions"
                )

            if overload.no_virtual_error_handler:
                error(
                    "/NoVirtualErrorHandler/ cannot be specified for non-virtual member functions"
                )
        ...
        if overload.factory:
            ...
            for arg in overload.py_signature.args:
                if arg.transfer is Transfer.TRANSFER_THIS:
                    error(
                        "/TransferThis/ may only be specified for constructors and member functions"
                    )
                    break
    ...
    def validate_mapped_type(self, p, symbol, mapped_type):
        ...
        if self.spec.target_abi is None or self.spec.target_abi >= (13, 0):
            convert_to_us = (
                mapped_type.convert_to_type_code is not None
                and "sipUserState" in mapped_type.convert_to_type_code.text
            )

            release_us = (
                mapped_type.release_code is not None
                and "sipUserState" in mapped_type.release_code.text
            )
            ...
            if mapped_type.convert_to_type_code is None:
                self.parser_error(
                    p, symbol, "%MappedType must have a %ConvertToTypeCode directive"
                )

            if mapped_type.convert_from_type_code is None:
                self.parser_error(
                    p, symbol, "%MappedType must have a %ConvertFromTypeCode directive"
                )
    ...
    def validate_variable(self, p, symbol, variable):
        ...
        if variable.type.type is ArgumentType.CAPSULE:
            self.parser_error(p, symbol, "capsule variables are not yet supported")

        access_specifier = self.scope_access_specifier
        if access_specifier is not None:
            if access_specifier is not AccessSpecifier.PUBLIC:
                self.parser_error(
                    p, symbol, "class variables must be in the public section"
                )
        ...
        if (
            ...
            and self.scope.iface_file.type is IfaceFileType.NAMESPACE
        ):
    ...
    def _add_auto_slot(
        self, p, symbol, annotations, py_name, py_signature, cpp_signature, method_code
    ):
        """Add an automatically generated slot."""

        member = self._find_member(
            p, symbol, py_name, py_signature.args, annotations, method_code
        )
    ...
    def _check_ellipsis(self, p, symbol, signature):
        ...
        for arg in signature.args:
            if arg.type is ArgumentType.ELLIPSIS:
                ...
                if seen_ellipsis:
                    self.parser_error(p, symbol, "'...' may only be specified once")
                    break

                seen_ellipsis = True
            elif seen_ellipsis and arg.default_value is None:
                self.parser_error(
                    p, symbol, "'...' must be at the end of the positional arguments"
                )
                break

    def find_iface_file(self, p, symbol, fq_cpp_name, iface_file_type, cpp_type=None):
    ...
    def _deprecated_target_abi(major_version, minor_version, instead):
        ...
        deprecated(
            f"ABI v{major_version}.{minor_version}", instead=f"v{instead} or later"
        )
    ...
    def _find_member(self, p, symbol, py_name, args, annotations, method_code):
                ...
                if self.scope_access_specifier is None:
                    ...
                    if invalid_global_slot(py_slot):
                        self.parser_error(
                            p,
                            symbol,
                            "'{0}' cannot be specified at the module level".format(
                                py_name
                            ),
                        )
        ...
        if self.scope is None:
            ...
            self.scope.iface_file.type is IfaceFileType.NAMESPACE
            and py_slot is not None
        ):
        ...
        for member in members:
            ...
            member = Member(self.module_state.module, cached_name(self.spec, py_name))
    ...
    def _find_timeline_qualifier(self, p, symbol):
        ...
        if qual.type is not QualifierType.TIME:
            self.parser_error(
                p, symbol, "'{0}' is not a %Timeline qualifier".format(name)
            )
            return None
    ...
    def _get_gil_action(self, p, symbol, annotations):
        ...
        if hold:
            if release:
                self.parser_error(
                    p, symbol, "/HoldGIL/ and /ReleaseGIL' cannot both be specified"
                )
    ...
    def _get_plugin_annotation(self, p, symbol, annotations, name, plugin):
        ...
        if anno is not None and plugin not in self.spec.plugins:
            self.parser_error(
                p, symbol, "/{0}/ is only supported for {1}".format(name, plugin)
            )
    ...
    def _handle_eom(self):
        ...
        if module.fq_py_name is None:
            self._error_log.log(
                "%Module has not been specified", SourceLocation(self._sip_file)
            )
    ...
    def _import_module(self, sip_file):
        ...
        mod_tags, mod_disabled = get_bindings_configuration(
            self.spec, sip_file, self._include_dirs
        )
    ...
    def _unexpected_eof_error(self):
        ...
        self._error_log.log("unexpected end of file", SourceLocation(self._sip_file))
