...
def p_release_buffer_code(p):
    ...
    if pm.scope.bi_release_buffer_code is not None:
        pm.parser_error(p, 1, "%BIReleaseBufferCode can only be specified once")
...
def p_convert_from_type_code(p):
    ...
    if pm.scope.convert_from_type_code is not None:
        pm.parser_error(p, 1, "%ConvertFromTypeCode can only be specified once")
...
def p_convert_to_subclass_code(p):
    ...
    if pm.scope.convert_to_subclass_code is not None:
        pm.parser_error(p, 1, "%ConvertToSubClassCode can only be specified once")
...
def p_defdocstringfmt(p):
    ...
    pm.module_state.module.default_docstring_format = pm.convert_docstring_format(
        p, symbol
    )
...
def p_defdocstringsig(p):
    ...
    pm.module_state.module.default_docstring_signature = pm.convert_docstring_signature(
        p, symbol
    )
...
def p_defmetatype(p):
    ...
    if module.default_metatype:
        pm.parser_error(
            p, 1, "%DefaultMetatype has already been defined for this module"
        )
...
def p_defsupertype(p):
    ...
    if module.default_supertype:
        pm.parser_error(
            p, 1, "%DefaultSupertype has already been defined for this module"
        )
...
def p_docstring(p):
    ...
    p[0] = Docstring(p[2].get("signature", module.default_docstring_signature), text)
...
def p_hidden_ns(p):
    ...
    if len(p) == 3:
        ...
        try:
            ...
            pm.parser_error(p, 1, "the name of the namespace has not been specified")
            name = ""

    namespace = pm.new_class(
        p, 1, IfaceFileType.NAMESPACE, normalised_scoped_name(name, pm.scope)
    )
...
def p_mapped_type_template_head(p):
    ...
    for mtt in pm.spec.mapped_type_templates:
        if mtt.mapped_type.type.definition.cpp_name == p[
            3
        ].definition.cpp_name and same_template_signature(
            mtt.mapped_type.type.definition.types, p[3].definition.types, deep=True
        ):
            pm.parser_error(
                p, 1, "a %MappedType template for this type has already been defined"
            )
...
def p_module(p):
    ...
    module_state.all_raise_py_exception = args.get("all_raise_py_exception", False)
    module.default_virtual_error_handler = args.get("default_VirtualErrorHandler")
...
def p_property(p):
    ...
    name = p[4].get("name")
    if name is None:
        p.parser.pm.parser_error(p, 1, "a name must be specified for %Property")
        return
    ...
    getter = p[4].get("get")
    if getter is None:
        p.parser.pm.parser_error(p, 1, "a getter must be specified for %Property")
        return
...
def p_timeline(p):
    ...
    for order, qual_name in enumerate(p[3]):
        ...
        pm.add_qualifier(
            p, 1, qual_name, QualifierType.TIME, order=order, timeline=timeline
        )
...
def p_virtual_error_handler(p):
    ...
    if len(p) == 4:
        ...
        name = p[4].get("name")
        if name is None:
            pm.parser_error(p, 1, "a name must be specified for %VirtualErrorHandler")
            return
    ...
    for handler in pm.spec.virtual_error_handlers:
        if handler.name == name:
            pm.parser_error(
                p,
                1,
                "a virtual error handler called '{0}' has already been defined".format(
                    name
                ),
            )
            break
...
def p_cpp_type(p):
    ...
    if len(p) == 5:
        ...
        if (
            value.type is ArgumentType.DEFINED
            and len(value.definition) == 1
            and value.definition.base_name == "PyObject"
            and len(value.derefs) == 1
...
def p_base_type(p):
    ...
    if isinstance(p[1], ArgumentType):
        ...
        p[0] = Argument(
            ArgumentType.TEMPLATE, definition=Template(p[1], Signature(args=p[3]))
        )
...
def p_cpp_types(p):
    ...
    if len(p) == 2:
        ...
        if p[1][-1].type == ArgumentType.ELLIPSIS:
            p.parser.pm.parser_error(
                p, 3, "an ellipsis must be at the end of the argument list"
            )
...
def p_class_docstring(p):
    ...
    if pm.scope.docstring is None:
        ...
        pm.parser_error(p, 1, "%Docstring has already been defined for this class")
...
def p_simple_superclass(p):
    ...
    if (
        ad.type is not ArgumentType.NONE
        or len(ad.derefs) != 0
        or ad.is_const
        or ad.is_reference
    ):
    ...
    p[0] = pm.find_class(
        p, 1, IfaceFileType.CLASS, ad.definition, tmpl_arg=pm.parsing_template
    )
...
def p_method_variable(p):
    ...
    if len(p) == 3:
        ...
        if isinstance(item, Overload):
            item.pyqt_method_specifier = (
                PyQtMethodSpecifier.SIGNAL
                if p[1] == "Q_SIGNAL"
                else PyQtMethodSpecifier.SLOT
            )
        else:
            pm.parser_error(
                p, 1, "a PyQt method specifier can only be applied to member functions"
            )
    else:
        item = p[1]
...
def p_enum_line(p):
    ...
    if len(p) == 5:
        ...
        p[0] = (
            p[1],
            cached_name(pm.spec, pm.get_py_name(p[1], p[3])),
...
def p_exception(p):
    ...
    if raise_code is None:
        pm.parser_error(p, 1, "%Exception must have a %RaiseCode sub-directive")
    ...
    if xd.iface_file.module is not None:
        pm.parser_error(p, 2, "an %Exception with this name has already been defined")
    ...
    if p[4].get("Default"):
        if module.default_exception is None:
            ...
            pm.parser_error(
                p, 1, "another %Exception has already been annotated with '/Default/'"
            )
...
def p_operator_cast_decl(p):
    ...
    if pm.scope is None:
        pm.parser_error(p, 1, "operator casts can only be specified as part of a class")
    ...
    if slot_name is None:
        ...
        for cast in pm.scope.casts:
            if cast.definition == cpp_type.definition:
                pm.parser_error(
                    p, 2, "operator cast has already been specified for this class"
                )
                break
        else:
...
def p_arg_type(p):
    ...
    if has_array:
        if has_array_size:
            pm.parser_error(p, 3, "/Array/ and /ArraySize/ cannot both be specified")
...
_NAMESPACE_ANNOTATIONS = ("PyQtNoQMetaObject",)
...
def p_namespace_head(p):
    ...
    namespace = pm.new_class(
        p, 1, IfaceFileType.NAMESPACE, normalised_scoped_name(p[1], pm.scope)
    )
...
def p_namespace_docstring(p):
    ...
    if pm.scope.docstring is None:
        ...
        pm.parser_error(p, 1, "%Docstring has already been defined for this namespace")
...
def p_typedef_decl(p):
    ...
    if type.type is ArgumentType.TEMPLATE:
        instantiated = pm.instantiate_class_template(
            p,
            name_symbol,
            fq_cpp_name,
            type.definition,
    ...
    if "Capsule" in annotations:
        ...
        if (
            type.type is ArgumentType.VOID
            and len(type.derefs) == 1
            ...
            pm.parser_error(
                p, annos_symbol, "/Capsule/ can only be specified for a void* type"
            )
...
def p_variable(p):
    ...
    variable = WrappedVariable(
        fq_cpp_name, pm.module_state.module, py_name, pm.scope, type
    )
