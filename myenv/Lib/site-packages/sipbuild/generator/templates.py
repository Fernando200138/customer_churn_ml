...
def same_template_signature(sig1, sig2, deep=False):
    ...
    for type1, type2 in zip(sig1.args, sig2.args):
        ...
        if type1.type is ArgumentType.DEFINED and type2.type is ArgumentType.DEFINED:
            if type1.is_reference != type2.is_reference or len(type1.derefs) != len(
                type2.derefs
            ):
                return False

        elif (
            type1.type is ArgumentType.TEMPLATE and type2.type is ArgumentType.TEMPLATE
        ):
            if not same_template_signature(
                type1.definition.types, type2.definition.types, deep=deep
            ):
                return False
...
def template_code_blocks(spec, used, proto_code_blocks, expansions):
    ...
    return [
        _template_code_block(spec, used, pc, expansions) for pc in proto_code_blocks
    ]


def template_expansions(template_names, instantiation_values, declared_names=None):
    ...
    for arg_nr, name_arg in enumerate(template_names.args):
        if name_arg.type is ArgumentType.DEFINED:
            ...
            if value_arg.type is ArgumentType.TEMPLATE and len(
                name_arg.definition.types.args
            ) == len(value_arg.definition.types.args):
                expansions.update(
                    template_expansions(
                        name_arg.definition.types,
