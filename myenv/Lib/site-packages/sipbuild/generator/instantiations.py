...
def instantiate_class(
    ...
    i_class.iface_file = pm.find_iface_file(p, symbol, fq_cpp_name, IfaceFileType.CLASS)
    i_class.iface_file.module = pm.module_state.module

    used = i_class.iface_file.used

    i_class.iface_file.type_header_code = template_code_blocks(
        pm.spec, used, proto_class.iface_file.type_header_code, expansions
    )
    ...
    if proto_class.type_hints is not None:
        i_class.type_hints = instantiate_type_hints(
            pm.spec, proto_class.type_hints, expansions
        )

    # Handle any flagged enums.
    if proto_class.pyqt_flags_enums is not None:
        i_class.pyqt_flags_enums = [
            template_string(s, expansions) for s in proto_class.pyqt_flags_enums
        ]
    ...
    for superclass in proto_class.superclasses:
        if isinstance(superclass, WrappedClass):
            klass = _superclass_from_class(
                superclass, p, symbol, tmpl_names, template, pm
            )
        else:
            klass = _superclass_from_template(
                superclass, p, symbol, tmpl_names, template, pm
            )
    ...
    # Handle the enums.
    _instantiate_enums(tmpl_names, proto_class, template, i_class, expansions, pm)

    # Handle the variables.
    _instantiate_vars(tmpl_names, proto_class, template, i_class, expansions, pm)

    # Handle the typedefs.
    _instantiate_typedefs(
        p, symbol, tmpl_names, proto_class, template, i_class, expansions, pm
    )

    # Handle the ctors.
    i_class.ctors = _instantiate_ctors(
        tmpl_names, proto_class, template, i_class, expansions, pm
    )

    # Handle the methods.
    i_class.members = _instantiate_methods(
        proto_class.members, i_class.iface_file.module, pm
    )
    i_class.overloads = _instantiate_overloads(
        proto_class.overloads,
        proto_class.members,
        i_class.members,
        tmpl_names,
        proto_class,
    ...
    i_class.bi_get_buffer_code = template_code(
        pm.spec, used, proto_class.bi_get_buffer_code, expansions
    )
    i_class.bi_release_buffer_code = template_code(
        pm.spec, used, proto_class.bi_release_buffer_code, expansions
    )
    i_class.convert_from_type_code = template_code(
        pm.spec, used, proto_class.convert_from_type_code, expansions
    )
    i_class.convert_to_subclass_code = template_code(
        pm.spec, used, proto_class.convert_to_subclass_code, expansions
    )
    i_class.convert_to_type_code = template_code(
        pm.spec, used, proto_class.convert_to_type_code, expansions
    )
    i_class.dealloc_code = template_code_blocks(
        pm.spec, used, proto_class.dealloc_code, expansions
    )
    i_class.dtor_virtual_catcher_code = template_code(
        pm.spec, used, proto_class.dtor_virtual_catcher_code, expansions
    )
    i_class.finalisation_code = template_code(
        pm.spec, used, proto_class.finalisation_code, expansions
    )
    i_class.gc_clear_code = template_code(
        pm.spec, used, proto_class.gc_clear_code, expansions
    )
    i_class.gc_traverse_code = template_code(
        pm.spec, used, proto_class.gc_traverse_code, expansions
    )
    i_class.instance_code = template_code(
        pm.spec, used, proto_class.instance_code, expansions
    )
    i_class.pickle_code = template_code(
        pm.spec, used, proto_class.pickle_code, expansions
    )
    i_class.type_code = template_code_blocks(
        pm.spec, used, proto_class.type_code, expansions
    )
    i_class.type_hint_code = template_code(
        pm.spec, used, proto_class.type_hint_code, expansions
    )
...
def _instantiate_argument(
    proto_arg, proto_class, tmpl_names, template, i_class, expansions, pm
):
    ...
    if proto_arg.default_value is not None:
        i_arg.default_value = [
            _instantiate_value(v, expansions) for v in proto_arg.default_value
        ]

    # Handle any type hints.
    if proto_arg.type_hints is not None:
        i_arg.type_hints = instantiate_type_hints(
            pm.spec, proto_arg.type_hints, expansions
        )
...
def _instantiate_ctors(tmpl_names, proto_class, template, i_class, expansions, pm):
    ...
    for proto_ctor in proto_class.ctors:
        ...
        if proto_ctor.cpp_signature is proto_ctor.py_signature:
            ...
            i_ctor.cpp_signature = _instantiate_signature(
                proto_ctor.cpp_signature.cpp_signature,
                proto_class,
                tmpl_names,
                template,
                i_class,
        ...
        i_ctor.method_code = template_code(
            pm.spec, used, proto_ctor.method_code, expansions
        )
        i_ctor.premethod_code = template_code(
            pm.spec, used, proto_ctor.premethod_code, expansions
        )
...
def _instantiate_enums(tmpl_names, proto_class, template, i_class, expansions, pm):
    ...
    for proto_enum in list(pm.spec.enums):
        ...
        if proto_enum.fq_cpp_name is not None:
            i_enum.fq_cpp_name = normalised_scoped_name(proto_enum.fq_cpp_name, i_class)
            i_enum.cached_fq_cpp_name = cached_name(pm.spec, str(i_enum.fq_cpp_name))
...
def _instantiate_overloads(
    ...
    for proto_overload in proto_overloads:
        ...
        i_overload.py_signature = _instantiate_signature(
            ...
            expansions,
            pm,
            kw_args=proto_overload.kw_args,
        )
        ...
        if proto_overload.cpp_signature is proto_overload.py_signature:
            ...
            i_overload.cpp_signature = _instantiate_signature(
                proto_overload.cpp_signature.cpp_signature,
                proto_class,
                tmpl_names,
                template,
                i_class,
        ...
        i_overload.method_code = template_code(
            pm.spec, used, proto_overload.method_code, expansions
        )
        i_overload.premethod_code = template_code(
            pm.spec, used, proto_overload.premethod_code, expansions
        )
        i_overload.virtual_call_code = template_code(
            pm.spec, used, proto_overload.virtual_call_code, expansions
        )
        i_overload.virtual_catcher_code = template_code(
            pm.spec, used, proto_overload.virtual_catcher_code, expansions
        )
...
def _instantiate_signature(
    ...
    for proto_arg in proto_signature.args:
        ...
        if pm.in_main_module and i_arg.name is not None:
            if kw_args is KwArgs.ALL or (
                kw_args is KwArgs.OPTIONAL and i_arg.default_value is not None
            ):
                i_arg.name.used = True

    if proto_signature.result is not None:
        i_signature.result = _instantiate_argument(
            proto_signature.result,
            proto_class,
            tmpl_names,
            template,
            i_class,
...
def instantiate_type_hints(spec, proto_type_hints, expansions):
    ...
    if proto_type_hints.hint_in is not None:
        hint_in = template_string(
            proto_type_hints.hint_in, expansions, scope_replacement="."
        )
    else:
        hint_in = None

    if proto_type_hints.hint_out is not None:
        hint_out = template_string(
            proto_type_hints.hint_out, expansions, scope_replacement="."
        )
    else:
        hint_out = None

    return TypeHints(
        hint_in=hint_in, hint_out=hint_out, default_value=proto_type_hints.default_value
    )


def _instantiate_typedefs(
    p, symbol, tmpl_names, proto_class, template, i_class, expansions, pm
):
    ...
    for proto_typedef in pm.spec.typedefs:
        ...
        i_typedef.type = _instantiate_argument(
            proto_typedef.type,
            proto_class,
            tmpl_names,
            template,
...
def _instantiate_value(proto_value, expansions):
    ...
    if (
        proto_value.value_type is ValueType.FCALL
        and proto_value.value.result.type is ArgumentType.DEFINED
    ):
        ...
        if proto_name.is_simple:
            ...
            i_fcall = FunctionCall(result=i_result, args=proto_value.value.args)
            i_value = Value(ValueType.FCALL, value=i_fcall)

    return i_value


def _instantiate_vars(tmpl_names, proto_class, template, i_class, expansions, pm):
    """Instantiate the enums for a template class."""
    ...
    for proto_var in pm.spec.variables:
        ...
        i_var.fq_cpp_name = normalised_scoped_name(proto_var.fq_cpp_name, i_class)
        i_var.scope = i_class
        i_var.module = i_class.iface_file.module

        i_var.type = _instantiate_argument(
            proto_var.type, proto_class, tmpl_names, template, i_class, expansions, pm
        )

        i_var.access_code = template_code(
            pm.spec, used, proto_var.access_code, expansions
        )
        i_var.get_code = template_code(pm.spec, used, proto_var.get_code, expansions)
        i_var.set_code = template_code(pm.spec, used, proto_var.set_code, expansions)
...
def _superclass_from_class(klass, p, symbol, tmpl_names, template, pm):
    ...
    if klass.iface_file.module is None and superclass_name.is_simple:
        superclass = _find_argument_value(
            superclass_name.base_name, p, symbol, tmpl_names, template, pm
        )
...
def _superclass_from_template(superclass, p, symbol, tmpl_names, template, pm):
    ...
    for proto_arg in proto_superclass_template.types.args:
        ...
        if proto_arg.type is ArgumentType.DEFINED and proto_arg.definition.is_simple:
            klass = _find_argument_value(
                proto_arg.definition.base_name, p, symbol, tmpl_names, template, pm
            )

            if klass is not None:
                superclass_arg = Argument(ArgumentType.CLASS, definition=klass)
                superclass_name_parts.append(klass.iface_file.fq_cpp_name.as_word)

        superclass_template_args.append(superclass_arg)

    superclass_template = Template(
        proto_superclass_template.cpp_name, Signature(args=superclass_template_args)
    )
    ...
    fq_cpp_name[-1] = _make_generated_name(fq_cpp_name[-1], superclass_name_parts)
    py_name = _make_generated_name(
        superclass_template.cpp_name.as_py, superclass_name_parts
    )

    # Note that any error will have a misleading location as source_location
    # should be used.
    klass = pm.instantiate_class_template(
        p,
        symbol,
        fq_cpp_name,
        superclass_template,
    ...
    if klass is None:
        pm.parser_error_at_location(
            superclass.source_location, "unknown class template"
        )
    ...
    for a, arg in enumerate(tmpl_names.args):
        if arg.definition.base_name == name:
            ...
            if tmpl_arg.type is ArgumentType.DEFINED:
                return pm.find_class(
                    p, symbol, IfaceFileType.CLASS, tmpl_arg.definition
                )
