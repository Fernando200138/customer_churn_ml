...
from .generator.outputs import output_api, output_code, output_extract, output_pyi
...
class Bindings(Configurable):
    ...
    _options = (
        ...
        Option(
            "disabled_features",
            option_type=list,
            help="disable the TAG feature tag",
            metavar="TAG",
        ),
        Option(
            "docstrings",
    ...
    def generate(self):
        ...
        if (
            project.target_abi is None
            or project.target_abi[1] is None
            or project.target_abi < spec.target_abi
        ):
        ...
        buildable = BuildableBindings(
            self, module.fq_py_name.name, uses_limited_api=uses_limited_api
        )
        ...
        if project.api_dir and not self.internal:
            project.progress("Generating the {0} .api file".format(buildable.target))

            output_api(spec, os.path.join(project.build_dir, buildable.target + ".api"))
        ...
        if self.pep484_pyi and not self.internal:
            project.progress("Generating the {0} .pyi file".format(buildable.target))

            pyi_path = os.path.join(buildable.build_dir, buildable.target + ".pyi")

            output_pyi(spec, project, pyi_path)

            installable = Installable(
                "pyi", target_subdir=buildable.get_install_subdir()
            )
        ...
        if project.sip_module:
            ...
            buildable.sources.extend(
                copy_nonshared_sources(project.build_abi, buildable.build_dir)
            )
    ...
    def verify_configuration(self, tool):
        ...
        self.extra_objects = [project.project_path(o) for o in self.extra_objects]
        self.headers = [project.project_path(h) for h in self.headers]
        self.include_dirs = [project.project_path(d) for d in self.include_dirs]
        self.library_dirs = [project.project_path(d) for d in self.library_dirs]
        self.sip_file = project.project_path(
            self.sip_file, relative_to=project.sip_files_dir
        )
        ...
        if sys.platform == "win32":
            if self.debug:
                ...
                raise UserException(
                    ...
                    "when a debug version of Python is used".format(self.name)
                )
