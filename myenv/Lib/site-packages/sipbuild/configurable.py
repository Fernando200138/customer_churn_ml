        ...
        for option in options:
            ...
            if option.option_type is bool:
                parser.add_argument(
                    argument_name,
                    dest=option.dest,
                    action=("store_false" if option.inverted else "store_true"),
                    help=option.help,
                )
            ...
            for name, value in section.items():
                ...
                for option in self.get_options():
                    ...
                    raise PyProjectOptionException(
                        name, "is not a supported option", section_name=section_name
                    )
                ...
                if not isinstance(value, option.option_type):
                    raise PyProjectOptionException(
                        ...
                        "should be of type '{0}' and not '{1}'".format(
                            option.option_type.__name__, type(value).__name__
                        ),
        ...
        if tools is None:
            ...
            for tool in tools:
                if tool not in self._ALL_TOOLS:
                    raise UserException(
                        "'{0}' option has an invalid tools '{1}'".format(name, tool)
                    )
