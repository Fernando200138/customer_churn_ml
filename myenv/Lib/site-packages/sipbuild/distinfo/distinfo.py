...
def create_distinfo(
    ...
    if os.path.exists(real_distinfo_dir):
        try:
            ...
            raise UserException(
                "unable remove old dist-info directory '{}'".format(real_distinfo_dir),
                str(e),
            )
    ...
    try:
        ...
        raise UserException(
            "unable create dist-info directory '{}'".format(real_distinfo_dir), str(e)
        )
...
"""
        ...
        with open(prefix_dir + wheel_fn, "w") as wheel_f:
            wheel_f.write(
                WHEEL.format(WHEEL_VERSION, generator, generator_version, wheel_tag)
            )

    # Create the METADATA file.
    metadata_fn = os.path.join(distinfo_dir, "METADATA")
    write_metadata(
        metadata, requires_dists, metadata_fn, project_root, prefix_dir=prefix_dir
    )
        ...
        for name in installed:
            ...
            for fn in all_fns:
                ...
                if norm_fn.startswith(real_distinfo_path):
                    ...
                    fn_name = os.path.relpath(fn, real_distinfo_path).replace("\\", "/")
                else:
                ...
                record_f.write("{},sha256={},{}\n".format(fn_name, digest, len(data)))

        record_f.write("{}/RECORD,,\n".format(distinfo_base))


def write_metadata(metadata, requires_dists, metadata_fn, project_root, prefix_dir=""):
