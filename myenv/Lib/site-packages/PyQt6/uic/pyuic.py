...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog=PROGRAM_NAME, description="Python User Interface Compiler"
    )
    ...
    try:
        if args.preview:
            ...
            generate(args.ui, args.output, args.indent, args.execute, args.max_workers)
            exit_status = 0

    except IOError as e:
        print("Error: {0}: '{1}'".format(e.strerror, e.filename), file=sys.stderr)
...
- the debug output of {name} (use the --debug flag when calling {name})""".format(
                    name=PROGRAM_NAME, version=PYQT_VERSION_STR
                ),
                file=sys.stderr,
            )
...
def generate(ui_file, output, indent, execute, max_workers):
    ...
    if os.path.isdir(ui_file):
        if output == "-":
            ...
            raise UIFileException(
                output, f"must be a directory as {ui_file} is a directory"
            )
        ...
        raise UIFileException(
            output, f"cannot be a directory unless {ui_file} is a directory"
        )
    else:
