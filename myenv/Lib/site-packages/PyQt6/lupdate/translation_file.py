        ...
        for context in source.contexts:
            ...
            for message in context.messages:
                ...
                if message_el is None:
                    ...
                    self.progress(
                        "Added new message '{0}'".format(self.pretty(message.source))
                    )
                    self._nr_new += 1
                else:
                    self.progress(
                        "Updated message '{0}'".format(self.pretty(message.source))
                    )
    ...
    def _summary(self):
        ...
        if self._nr_new:
            if self._nr_new_duplicates:
                summary(
                    "{0} new messages were added (and {1} duplicates)".format(
        ...
        if self._nr_new_using_existing_translation:
            summary(
                "{0} messages reused existing translations".format(
                    self._nr_new_using_existing_translation
                )
            )

        if self._nr_existing:
            summary("{0} existing messages were found".format(self._nr_existing))

        if self._nr_kept_obsolete:
            summary("{0} obsolete messages were kept".format(self._nr_kept_obsolete))

        if self._nr_discarded_obsolete:
            summary(
                "{0} obsolete messages were discarded".format(
        ...
        if self._nr_discarded_untranslated:
            summary(
                "{0} untranslated messages were discarded".format(
