...
def lupdate(
    ...
    translations = [
        TranslationFile(
            ts, no_obsolete=no_obsolete, no_summary=no_summary, verbose=verbose
        )
    ...
    for source in sources:
        if os.path.isdir(source):
            for dirpath, dirnames, filenames in os.walk(source):
                ...
                for fn in filenames:
                    ...
                    if filename.endswith(".py"):
                        source_files.append(
                            PythonSource(filename=filename, verbose=verbose)
                        )

                    elif filename.endswith(".ui"):
                        source_files.append(
                            DesignerSource(filename=filename, verbose=verbose)
                        )
            ...
            raise UserException(
                "{0} must be a directory or a .py or a .ui file".format(source)
            )
